...
- **Distribution notes recorded by D.** `/chat/release.json` is a deployment-visible contract read by `update`; the production plugin channel stays manual dispatch while release workflows publish the canary channel; `publish-cli.yml` and `release-cli.yml` remain two consistent paths; update downloads from the fixed GitHub release base, so self-hosted deployments cannot serve their own assets yet.
