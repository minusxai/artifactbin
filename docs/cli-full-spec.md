...
The eval now has two treatments (`evals/lib/mode.ts`): `installed`, the runs above, and `cold`, where only the CLI is staged and the agent runs `afbin setup`; the task proxy rewrites the device door's advertised origin to itself and the driver approves the pairing with its own session (`evals/lib/approver.ts`). Probe on 2026-09-11: pi on DeepSeek v4 flash, `--mode cold --tasks edit`, approved one pairing and passed in 46 seconds over 12 turns, so the forgiving-auth path is exercised headlessly with the agent never seeing a token.
