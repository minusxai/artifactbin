...
| `--type <artifact\|folder\|dataset\|file\|session\|comment>` | P | Select the resource type; default inferred/artifact. [^delete] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Infer from typed file/tracking; otherwise use the documented command default. Conflicts → error, never reinterpret. Enum values ignore case. [^syntax] |
