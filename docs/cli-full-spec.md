...
Revision 2, 2026-09-11. The command table and its normative footnotes define the required final behavior. Revision 2 adds a verified baseline, resolves every design choice the first revision left to the primary, seeds the parser and failing tests, and assigns four bounded workstreams. Nothing in this document claims evidence that was not observed; each "Observed" line names the command that produced it.
...
| Account and resource contracts | `services/contracts/src/account-resource.ts` (profile, token, session), `resource-file.ts` (`forked_from` added), `services/utils/src/account-resource.ts`, app `lib/account-profile.ts`, `lib/tokens.ts`, `lib/relations.ts`, `lib/remote/registry.ts`, `lib/trash.ts`, `lib/workspace.ts`, `lib/feed.ts`. |
...
- Contracts: `forked_from` on resource metadata and the JSX fence; `SessionResource` and `ACCOUNT_RESOURCE_TYPES`; the account resource parser accepts `session`.
- Server: `ACCOUNT_OPERATIONS` and `SESSION_OPERATIONS` seams spread into `OPERATIONS`.
- Filters: `list --filter state=active|deleted` parses.
...
`--type` values are one vocabulary: `artifact`, `folder`, `dataset`, `file` (artifact resources tracked in `afbin.lock`), `profile`, `token`, `session` (account resources tracked in `.artifactbin/accounts.json`). `list` adds the read-only collections `user`, `table`, `activity`, `analytics`; `delete` adds `comment`. There is no `connection` type: a connection is an inline part of a dataset definition, not an addressable resource. Mixed batches of artifact and account resources are allowed everywhere; the current `mixed_resource_batch` refusals are removed by workstream A, and each file is routed by its own tracking or typed YAML.
...
### Tokens, sessions, trash, activity and analytics

Token listing, creation and revocation are session-only on the server today. Workstream A adds bearer operations `list_account_tokens`, `create_account_token` and `revoke_account_token` in `lib/operations/account.ts`, reusing `lib/tokens.ts` and `lib/users.ts`. Creation returns the plaintext once; the CLI prints it once in the command result (or writes it to `--output` with mode 0600) and stores a redacted journal record. Listing never includes plaintext.

Sessions map to the existing bearer registry routes: `list --type session`, `pull --type session` (read-only YAML), `delete --type session` (terminate). No rename exists. `remote --session <id>` attaches as a controller mirror using the view, input and control routes; when a local recovery key for that id exists under `~/.artifactbin/remote/`, the CLI re-attaches as the runner instead. A removed session's 410 is reported, never replaced.

Trash is `list --filter state=deleted`, a new owner-scoped filter on `list_artifacts`. Restore is `push --restore <id>` through the existing bearer restore route. Activity is `list --type activity --filter feed=mine|following`; analytics is `list --type analytics`; both are new read-only bearer operations backed by `lib/workspace.ts` and `lib/feed.ts`. Like and follow stay the `liked` and `following` lists in profile YAML. Permanent delete, followers and liked-by lists, token rename and session rename are not offered; the product lacks them.
...
| --- | --- | --- | --- | --- |
| A. Resources | opus-implementer | `services/cli/src/account-workspace.ts`, `resource-file.ts`, `delete.ts`, new `services/cli/src/tokens.ts`, `sessions.ts`, `collections.ts`; app `lib/operations/account.ts`, `lib/operations/sessions.ts`, `lib/operations/http.ts` allowlist, `lib/tokens.ts`, `lib/users.ts`, `lib/trash.ts`, `lib/workspace.ts`, `lib/feed.ts`, `lib/artifacts.ts` (`state` filter), `lib/remote/*`; new routes under `app/api/account/*`; `__tests__/cli-account-resources.test.ts`, new `cli-tokens.test.ts`, `cli-sessions.test.ts`, `cli-trash.test.ts` | `seed-resources.test.ts` | `--type token|session` on pull/push/status/diff/delete; `list --type token|session|user|activity|analytics`; `list --filter state=deleted`; `push --restore`; `push --refresh`; `delete --type folder|dataset|file|session|token` with multiple targets; mixed batches; durable recovery for all of these |
...
- **A.** Real-handler tests prove: tokens listed by metadata only; creation delivers plaintext once and journals a redacted record; a retried create or revoke with the same operation identity produces one effect; viewer, editor and owner behavior matches the UI doors; trash listing is owner-scoped; restore of a live row is reported as already restored, never as failure; refresh reports per-target outcomes; sessions terminate once; activity and analytics equal the session-door values for the same user. Mixed workspaces track both resource families and every `mixed_resource_batch` throw is gone.
...
| Token list/create/revoke | Bearer door mints via operator secret only (`app/api/tokens/route.ts`); list and revoke are session-only (`app/api/my/tokens`) | New bearer operations in `lib/operations/account.ts` reusing `lib/tokens.ts:86,198,259` and `lib/users.ts:438`. |
...
| Trash view | Session-only `GET /api/page/trash` (`lib/trash.ts:152`) | New `state=deleted` filter on `list_artifacts`, owner scope. |
...
| Activity, analytics | Session-only `GET /api/page/home?part=insights` (`lib/workspace.ts:31`) | New read-only bearer operations `list_account_activity`, `read_account_analytics`. |
...
| Trash list, restore (`Trash.tsx`) | `list --filter state=deleted`, `push --restore <id>` | A |
...
| Dashboard metrics and engagement series (`Dashboard.tsx`) | `list --type analytics` | A |
| Activity feed (`ActivityFeed.tsx`) | `list --type activity --filter feed=mine|following` | A |
| Shared with you (`SharedWithYou.tsx`) | `list --filter relationship=shared` | Exists |
| Token list, revoke, mint with expiry, one-time display (`TokensPanel.tsx`, `TokensNew.tsx`) | `list`, `delete`, `push --type token` | A |
...
| --- | --- |
| Local-first | With network disabled and an isolated home: help, version, validate, status, diff, local SQL, fork of a local source, data-format export of a local file and an unchanged ordinary push succeed without auth, polling or unexpected writes. |
...
| `--version` | P | Print the installed CLI version and protocol version. [^global] | **Partial** — Offline; protocol is included with --json, but plain output shows only CLI version. | Always offline; no update check. |
...
| `afbin pull [<ref> ...]` | C | Retrieve editable files and reconcile tracked changes using shared node-scoped merge rules. [^pull] [^latest-main] | **Partial** — Shared merge, conflict records, force backups and native YAML source tracking exist; stdout, conversion, dataset definitions and token/session kinds are seeded. | No refs → tracked files. Merge unrelated edits; retain local work on conflict. [^defaults] |
| `--type <artifact\|folder\|dataset\|file\|profile\|token\|session>` | C/A | Interpret the targets as the selected resource type; default `artifact`. [^pull] | **Partial** — Profile workflow exists with focused tests; other accepted type values need consistent enforcement; token/session remain. | Infer from typed file/tracking; otherwise use the documented command default. Conflicts → error, never reinterpret. Enum values ignore case. [^syntax] |
...
| `afbin fork <ref> [<ref> ...]` | B | Create a distinct private local draft with recorded lineage; publish it later with push. [^fork] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Require source; suggest a free destination. Default to private; never publish implicitly. |
| `--type <artifact\|folder\|dataset\|file>` | B | Select the source resource type when it cannot be inferred. [^fork] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Infer from typed file/tracking; otherwise use the documented command default. Conflicts → error, never reinterpret. Enum values ignore case. [^syntax] |
| `-o, --output <path>` | B | Destination for the new editable draft; defaults to a collision-checked suggested filename. [^fork] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Choose a free destination; never overwrite a source or existing draft. |
| `-n, --dry-run` | B | Show new local files, retained dependencies, and sharing defaults without creating files or resources. [^fork] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | No side effects or auth setup; report missing credentials/capabilities clearly. |
| `afbin export <ref> [<ref> ...]` | B | Export images or HTML of published heads through the server, or data and original bytes locally. [^export] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Infer format from destination; choose a free filename. Missing renderer gets an actionable error; never publish implicitly. |
| `--type <artifact\|folder\|dataset\|file>` | B | Select the resource type when it cannot be inferred. [^export] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Infer from typed file/tracking; otherwise use the documented command default. Conflicts → error, never reinterpret. Enum values ignore case. [^syntax] |
| `--format <png\|jpg\|html\|csv\|json\|yaml\|original>` | B | Select the export representation. [^export] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Infer only where documented; reject unsupported or contradictory representations locally. Enum values ignore case. [^syntax] |
| `-o, --output <path\|->` | B | Write exported content; default to a collision-checked filename. [^export] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Suggest a free destination; reject ambiguous multiple outputs and unrelated-file overwrite. |
| `--name <name>` | B | Select a named table/query result, with the same meaning as in `query`. [^export] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Require an existing name when selecting; explain available names. No guessed query/mutation. |
| `--page <n>` | B | Select a 1-based slide/page for a paginated document, with the same meaning as in `open`. [^export] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | 1-based; reject out-of-range values with valid range. Omitted → documented whole-resource view/export. |
| `-f, --force` | B | Replace an existing untracked export destination after preserving a recoverable copy. [^export] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Only explicit overwrite permission; preserve recovery and enforce authorization. |
| `-n, --dry-run` | B | Validate export inputs and report destinations and required capabilities without rendering or writing output. [^export] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | No side effects or auth setup; report missing credentials/capabilities clearly. |
...
| `--type <artifact\|folder\|dataset\|file\|profile\|token\|session>` | A | Select the type when it cannot be determined from a typed file or tracking. [^push] | **Partial** — Profile workflow exists; consistent enforcement and token/session remain. | Infer from typed file/tracking; otherwise use the documented command default. Conflicts → error, never reinterpret. Enum values ignore case. [^syntax] |
| `--secret-env <NAME>` | C | Read a secret value (a dataset connection password) from the named environment variable; create the bound secret and reference its id. [^push] | **Seeded** — Parser accepts it; dispatch refuses until C lands it. | Never write the value to YAML, definitions, journals or output; the definition references the returned secret id. |
| `--restore` | A | Restore the selected soft-deleted resources, preserving identity and applicable permissions. [^push] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Require explicit targets; retry the same restore safely. Never infer bulk restoration. |
| `--refresh` | A | Refresh the selected resources' declared external data or imported assets. [^push] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Require explicit targets; report changed/unchanged/failed resources independently. |
...
| `--remote` | P | Additionally check server-dependent constraints without persisting a preview or mutation. [^validate] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Omitted → local observations where supported; requested → refresh only. |
| `afbin status [<ref> ...]` | P | Report local changes, conflicts and installation state; refresh remote observations only when requested. [^status] [^latest-main] | **Partial** — Default workspace status is local; positional targets seeded; installation/account summary pending. | No refs → workspace/installation summary. No workspace → useful setup status; no authentication just to report status. |
| `--type <artifact\|folder\|dataset\|file\|profile\|token\|session>` | A | Restrict the tracked resources being reported. [^status] | **Partial** — Profile tracking exists; mixed workspaces and full resource filtering remain. | Infer from typed file/tracking; otherwise use the documented command default. Conflicts → error, never reinterpret. Enum values ignore case. [^syntax] |
...
| `-o, --output <path\|->` | C | Write the diff; defaults to stdout. [^diff] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Default to stdout; explicit file output must not overwrite unrelated content silently. |
...
| `--type <artifact\|folder\|dataset\|file\|user\|table\|token\|session\|activity\|analytics>` | A | Select a resource collection or read-only view; default `artifact`. [^list] | **Partial** — artifact/folder/dataset/file implemented; profile/token/session/user/table/activity/analytics seeded. | Infer from typed file/tracking; otherwise use the documented command default. Conflicts → error, never reinterpret. Enum values ignore case. [^syntax] |
...
| `--type <artifact\|folder\|dataset\|file>` | C | Select a versioned resource type; default inferred/artifact. [^log] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Infer from typed file/tracking; otherwise use the documented command default. Conflicts → error, never reinterpret. Enum values ignore case. [^syntax] |
| `--filter <field=value>` | C | Filter supported history fields such as author or date interval. [^log] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | No filters → documented collection default; invalid fields/values → supported choices. |
...
| `afbin delete <ref> [<ref> ...]` | A | Delete resources, revoke tokens or terminate sessions; preserve local source files. [^delete] | **Partial** — Single-artifact deletion preserves source files and removes tracking; multiple targets and typed actions seeded; durable recovery pending. | Require explicit targets; preserve local files. Resume confirmed same-operation retries; never infer delete-all. [^defaults] |
| `--type <artifact\|folder\|dataset\|file\|token\|session\|comment>` | P | Select the resource type; default inferred/artifact. [^delete] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Infer from typed file/tracking; otherwise use the documented command default. Conflicts → error, never reinterpret. Enum values ignore case. [^syntax] |
| `--in <ref>` | P | Identify the containing artifact when deleting comments. [^delete] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Infer only from unambiguous tracking; otherwise require the containing resource. |
...
| `-n, --dry-run` | P | Check anchors, thread state, permissions, and proposed changes without posting. [^comment] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | No side effects or auth setup; report missing credentials/capabilities clearly. |
...
| `-n, --dry-run` | C | Validate a requested mutation and report its planned effect without applying it. [^query] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | No side effects or auth setup; report missing credentials/capabilities clearly. |
...
| `afbin open <ref> [<ref> ...]` | B | Open the published view of a resource, or print its URL with `--no-browser`; there is no local draft preview. [^open] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Local path → local preview; remote ref → published view. Failed browser launch → usable URL. |
| `--no-browser` | B | Return preview/view URLs without launching a browser. [^open] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Return a usable URL without launching a browser; preserve preview/session lifecycle. |
...
| `afbin update` | D | Explicitly update the compatible binary and skills; report plugin refresh requirements. [^update] | **Partial** — Explicit compatible binary/skill update with checksum verification and backups exist; plugin reporting and dry-run seeded. | Reuse saved harnesses; already current → success. Verify downloads and preserve recoverable installation. [^defaults] |
...
| `-n, --dry-run` | D | Resolve the compatible release and report binary/skill changes without installing them. [^update] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | No side effects or auth setup; report missing credentials/capabilities clearly. |
...
| `--session <ref>` | P | Attach to an existing authorized session instead of launching a new command; mutually exclusive with a command and `--name`. [^remote] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Attach to the exact authorized session; never create a replacement silently. |
| `--no-browser` | P | Print the session URL without opening a browser. [^remote] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Return a usable URL without launching a browser; preserve preview/session lifecycle. |
| `afbin help [<command-or-topic>]` | D | Read bundled command, schema and authoring guidance offline. [^help] | **Partial** — Bundled offline help exists and no longer teaches afbin api; schemas and generated formats pending. | No topic → command list. Always offline; unknown topic → valid alternatives. |
| `--format <text\|markdown\|man>` | D | Select a bundled documentation representation. [^help] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Infer only where documented; reject unsupported or contradictory representations locally. Enum values ignore case. [^syntax] |
| `-o, --output <path\|->` | D | Write the requested help; defaults to stdout. [^help] | **Seeded** — Parser accepts it; dispatch refuses with `command_integration_pending` until the owner lands it. | Default to stdout; reject conflicting output modes or ambiguous multiple results. |
...
| `afbin pull <ref> <destination>` (old positional form) | — | Replace destination positional with --output. [^removals] [^latest-main] | **Partial** — Shared merge, conflict records, force backups and native YAML source tracking exist; stdout, conversion, dataset definitions and token/session kinds are seeded. | Retired destinations are no longer interpreted; use --output. |
...
[^pull]: **`afbin pull` — behavior and required work.**
    ...
    **`--type <artifact\|folder\|dataset\|file\|profile\|token\|session>`:** Interpret the targets as the selected resource type; default `artifact`. Secret and bearer values are never returned; token YAML holds metadata only.
...
[^push]: **`afbin push` — behavior and required work.**
    ...
    **Acceptance / remaining work:** Retain node-scoped rebasing when metadata accompanies edits; commit content/governance atomically. Add typed folders, dataset definitions, token creation, restore and refresh; session YAML is read-only. Define explicit secret input/output and one-time token delivery without journaling secrets. Extend durable recovery to every mutation and handle local edits made while a response is in flight without losing either writer.

    **`--type <artifact\|folder\|dataset\|file\|profile\|token\|session>`:** Select the type when it cannot be determined from a typed file or tracking. A conflicting file type is an error. `session` accepts no push; terminate with `delete --type session`.
...
[^status]: **`afbin status` — behavior and required work.**
    ...
    **`--type <artifact\|folder\|dataset\|file\|profile\|token\|session>`:** Restrict the tracked resources being reported.
...
[^list]: **`afbin list` — behavior and required work.**
    ...
    **`--type <artifact\|folder\|dataset\|file\|user\|table\|token\|session\|activity\|analytics>`:** Select a resource collection or read-only view; default `artifact`. Analytics includes the metrics and time-series data available to the same user in the UI.
...
[^delete]: **`afbin delete` — behavior and required work.**
    ...
    Soft-delete artifacts, folders, datasets and files, revoke tokens, terminate remote sessions, or delete comments according to the selected type. Preserve local source files. Never interprets omitted references as “delete everything.” Each result states the domain action taken.
