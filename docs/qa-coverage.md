...
| Real agent comprehension of the CLI and local skills | Conditional agent-smoke matrix | API tests do not prove an agent can follow the shipped instructions |
...
| Former MCP domain matrix | `advanced-http-parity.test.ts` exercises the real HTTP handlers; app-flow retains built-server create/get |
...
and a real HTTP create/get roundtrip. These assembled-server checks are not implied by handler tests.
