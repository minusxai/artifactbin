...
and secrets are available to the trusted job. Its two modes are the two flows of the CLI. `installed`: the driver stages afbin on PATH and runs `afbin auth` in the run home before the harness starts, approving the browser pairing with its own session (`evals/lib/approver.ts`, `evals/lib/auth.ts`), so the CLI itself saved the connection and eager init installed the skills. `not-installed`: nothing is staged; the agent must find the installer from the server, run it and run any server command, whose first-use `afbin auth` the driver approves the same way; the task proxy serves the installer and this checkout's built release (`evals/lib/proxy.ts`). Both flows get the same prompt. Skills only ever arrive through the CLI; there is no plugin, MCP or staged-skill treatment. The token-less task gets no approver in either flow.
