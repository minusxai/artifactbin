...
useAppHarness();
const cacheRequest={cacheKey:'full:test',artifactId:'abc123',revision:'r1'};
...
describe('shared export refresh leases',()=>{
 it('combines cold requests across independent app instances without holding a transaction',async()=>{
  ...
  const first=a.read(cacheRequest,produce);void first.catch(()=>{});await Promise.race([entered.promise,first]);
  const second=b.read(cacheRequest,produce);
 ...
 it('serves stale immediately while one owner refreshes, then shares the replacement',async()=>{
  const db=await getDb(),a=createExportCache(db,options),b=createExportCache(db,options);
  const old=await a.read(cacheRequest,async()=>image);
  ...
  expect((await a.read(cacheRequest,produce)).id).toBe(old.id);
  expect((await b.read(cacheRequest,produce)).id).toBe(old.id);
  release.resolve();await a.drain();await b.drain();
  expect((await b.read(cacheRequest,produce)).object_key).toBe('exports/objects/new');expect(calls).toBe(1);
 });
 it('retains a successful image after failure and backs off subsequent refreshes',async()=>{
  ...
  const old=await cache.read(cacheRequest,async()=>image);let calls=0;
  const fail:ExportProducer=async()=>{calls++;throw new Error('render timed out');};
  const changed={...cacheRequest,revision:'r2'};
 ...
 it('fences a slow worker after lease recovery so it cannot overwrite the winner',async()=>{
  ...
  const first=a.read(cacheRequest,async()=>{entered.resolve();await release.promise;return image;});void first.catch(()=>{});await Promise.race([entered.promise,first]);
  await db.query("UPDATE export_image_cache SET lease_until=clock_timestamp()-interval '1 second'");
  const winner=await b.read(cacheRequest,async()=>({...image,object_key:'exports/objects/winner'}));
  release.resolve();expect((await first).id).toBe(winner.id);
  expect((await a.read(cacheRequest,async()=>image)).id).toBe(winner.id);
 });
 it('an explicit refresh waits for a new result and concurrent refresh callers share it',async()=>{
  const db=await getDb(),a=createExportCache(db,options),b=createExportCache(db,options);
  const old=await a.read(cacheRequest,async()=>image),entered=gate(),release=gate();let calls=0;
  const produce:ExportProducer=async()=>{calls++;entered.resolve();await release.promise;return image;};
  const first=a.read({...cacheRequest,refresh:true},produce);await Promise.race([entered.promise,first]);
  const second=b.read({...cacheRequest,refresh:true},produce);
