...
import { DELETE as unlike, GET as likeState, POST as like } from '@/app/api/artifacts/[id]/like/route';
...
describe('POST/DELETE/GET /api/artifacts/[id]/like', () => {
  it('a signed-in reader likes, sees the count, unlikes; anonymous is 401; an unreadable artifact is 404', async () => {
    ...
    expect((await like(request('/api/artifacts/art0pu/like', { method: 'POST', origin: 'same' }), ctx('art0pu'))).status).toBe(401);

    const liked = await like(request('/api/artifacts/art0pu/like', { method: 'POST', actor: session(fan), origin: 'same' }), ctx('art0pu'));
    expect(liked.status).toBe(200);
    expect(await liked.json()).toEqual({ liked: true, count: 1 });
    const again = await like(request('/api/artifacts/art0pu/like', { method: 'POST', actor: session(fan), origin: 'same' }), ctx('art0pu'));
    expect(await again.json()).toEqual({ liked: true, count: 1 });

    const state = await likeState(request('/api/artifacts/art0pu/like', { actor: session(fan) }), ctx('art0pu'));
    expect(await state.json()).toEqual({ liked: true, count: 1 });
    const anonymousState = await likeState(request('/api/artifacts/art0pu/like'), ctx('art0pu'));
    expect(await anonymousState.json()).toEqual({ liked: false, count: 1 });

    const unliked = await unlike(request('/api/artifacts/art0pu/like', { method: 'DELETE', actor: session(fan), origin: 'same' }), ctx('art0pu'));
    expect(await unliked.json()).toEqual({ liked: false, count: 0 });

    expect((await like(request('/api/artifacts/art0pr/like', { method: 'POST', actor: session(fan), origin: 'same' }), ctx('art0pr'))).status).toBe(404);
    expect((await like(request('/api/artifacts/nope00/like', { method: 'POST', actor: session(fan), origin: 'same' }), ctx('nope00'))).status).toBe(404);
  });
});
