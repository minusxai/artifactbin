/**
 * The dashboard's two view queries read the LOG now (lib/workspace-analytics.ts), and the
...
import { dailyViewsByUser, eventsTablePresent, viewSeriesByUser, VIEW_SERIES_DAYS } from '@/lib/workspace-analytics';
...
describe('the backfill and the dual-write do not say the same thing twice', () => {
  it('a moment that was emitted live is not copied again — the events log stores it once', async () => {
    const db = await harness.db();
    const queryable: Queryable = { query: async <T = Record<string, unknown>>(sql: string, params: unknown[] = []) => ({ rows: (await db.query<T>(sql, params)).rows }) };
    setServices({ events: createEvents({ db: queryable, schema: EVENTS_SCHEMA }) });
    await db.query(`INSERT INTO artifacts (id, token_id, user_id, content) VALUES ('art0a1', 'tok_a', 'usr_a', 'x')`);
    ...
    const rows = (await db.query(`SELECT * FROM ${EVENTS_SCHEMA}.events`)).rows;
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ verb: 'created', object_id: 'art0a1', subject_kind: 'user', subject_id: 'usr_a' });
    expect(String(rows[0].id).startsWith('legacy:')).toBe(false);
  });
});
