...
it('withholds the whole response when access is revoked after one source query has already succeeded',async()=>{
  const {id}=await fixture(true);const db=await harness.db();await db.query('DELETE FROM dataset_result_cache');
