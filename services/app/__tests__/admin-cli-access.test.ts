...
it('ordinary afbin pull/push works across owners through proxy and real handlers, and loses access on verification, allowlist or token revocation',async()=>{
 ...
 const proxy=createProxy({upstream:inProcess(app),tokens:createTokenReader({db}),sessions:{resolve:async()=>null,identity:async()=>identity},cookieSecret:'mxmx_test_cookie_secret_0123456789',env:{PROXY__RATE_LIMIT_CONFIG_FILE:resolve(__dirname,'../../proxy/dev_rate_limits.yml')}});
 ...
 try {
  ...
  identity={...identity,emailVerified:true};vi.stubEnv('ADMIN__EMAILS','');expect(await cli.invoke(['pull','repair.jsx'],respond)).not.toBe(0);expect(cli.last().error.code).toBe('not_found');
  vi.stubEnv('ADMIN__EMAILS','admin@example.com');await revokeToken(token.id);expect((await proxy.fetch(request('/api/artifacts/abc123',{token:token.token}))).status).toBe(401);expect(await cli.invoke(['pull','repair.jsx'],respond)).not.toBe(0);
