...
const recorded=async()=> (await (await harness.db()).query<{envelope:EventEnvelope}>("SELECT envelope FROM event_outbox WHERE envelope->>'verb'<>'notification_changed' ORDER BY created_at,id")).rows.map(r=>r.envelope);
...
describe('like', () => {
  it('link inserts one live edge and says artifact.liked once; linking again changes nothing and says nothing', async () => {
    ...
    expect((await recorded())[0]).toMatchObject({ verb: 'liked', subject_kind: 'user', subject_id: 'usr_a', object_kind: 'artifact', object_id: 'art001', source: 'app' });
  });
  it('unlink sets deleted_at on the same row and says unliked; unlinking again is absent and silent; a re-link revives the SAME row', async () => {
    ...
    expect((await recorded()).map((e) => e.verb)).toEqual(['liked', 'unliked', 'liked']);
  });
...
describe('follow', () => {
  it('the same module, the other vocabulary: user → user, user.followed / user.unfollowed', async () => {
    ...
    expect((await recorded()).map((e) => [e.verb, e.object_kind, e.object_id])).toEqual([['followed', 'user', 'usr_b'], ['followed', 'user', 'usr_c'], ['unfollowed', 'user', 'usr_c']]);
  });
