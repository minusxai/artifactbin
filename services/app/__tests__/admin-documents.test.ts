...
const documentPath = '/api/admin/documents/abc123';
const adminOptions = {actor:admin, origin:'same', headers:{'X-Artifactbin-Admin':'1'}};
...
describe('explicit email-authorized admin repairs',()=>{
  it('requires verified identity and explicit mode, and defaults to disabled',async()=>{
    for(const actor of [{...admin,emailVerified:false},{...admin,email:'other@evil.test'},{credential:'none' as const},{...admin,credential:'agent-cookie' as const}])expect((await GET(request(documentPath,{...adminOptions,actor}),context)).status).toBe(404);
    expect((await GET(request(documentPath,{...adminOptions,headers:{}}),context)).status).toBe(404);
    vi.stubEnv('ADMIN__EMAILS','');expect((await GET(request(documentPath,adminOptions),context)).status).toBe(404);
  });
  it('reads another owner’s source and audits the actual operator',async()=>{
    const response=await GET(request(documentPath,adminOptions),context);
  ...
  it('repairs atomically, preserves ownership and records version history under the admin',async()=>{
    const response=await PUT(request(documentPath,{...adminOptions,method:'PUT',json:{edit_id:'base-edit',source:'<p id="intro">After</p>',reason:'Repair legacy script'}}),context);
    ...
    expect((await PUT(request(documentPath,{...adminOptions,method:'PUT',json:{edit_id:'base-edit',source:'<p>stale</p>',reason:'retry'}}),context)).status).toBe(409);
  });
  it('refuses invalid source, missing reason and metadata changes without updating the document',async()=>{
    for(const body of [{edit_id:'base-edit',source:'<p>Changed</p>'},{edit_id:'base-edit',source:'<script>alert(1)</script>',reason:'repair'},{edit_id:'base-edit',source:'<p>Changed</p>',reason:'repair',visibility:'public'}])expect((await PUT(request(documentPath,{...adminOptions,method:'PUT',json:body}),context)).status).toBe(400);
  ...
  it('rolls source, history and identity back when audit persistence fails',async()=>{
    ...
    try {
      await expect(PUT(request(documentPath,{...adminOptions,method:'PUT',json:{edit_id:'base-edit',source:'<p id="intro">After</p>',reason:'repair'}}),context)).rejects.toThrow();
