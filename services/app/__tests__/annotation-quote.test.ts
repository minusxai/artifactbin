...
describe('an area comment', () => {
  ...
  const areaList = async (token: string, id: string): Promise<AreaWire[]> => {
    const res = await listAnnotationsRoute(request(`/api/artifacts/${id}/annotations`, { token }), params({ id }));
    expect(res.status).toBe(200);
    const body = (await res.json()) as { annotations?: AreaWire[] } | AreaWire[];
    return Array.isArray(body) ? body : body.annotations ?? [];
  };

  it('stores the box as the range and reads it back with no quote', async () => {
    const w = await areaSetup();
    const res = await areaComment(w, { range: AREA });
    expect(res.status, await res.clone().text()).toBe(201);
    const made = (await res.json()) as AreaWire;
    ...
    const [listed] = await areaList(w.t.token, w.doc.id);
    ...
    const inlined = (await head.json()) as { annotations: AreaWire[] };
  ...
  it('refuses a box outside the grammar by name, and never takes a quote with an area', async () => {
    const w = await areaSetup();
    const bad = await areaComment(w, { range: { v: 1, kind: 'area', box: { x: 0.8, y: 0.1, w: 0.5, h: 0.4 } } });
    expect(bad.status).toBe(400);
    expect(((await bad.json()) as { error: string }).error).toBe('bad_range');
    const missing = await areaComment(w, { range: { v: 1, kind: 'area' } });
    expect(missing.status).toBe(400);
    // A quote beside an area is a contradiction: the area has no words. Refused, not silently dropped.
    const both = await areaComment(w, { range: AREA, quote: 'Revenue grew' });
