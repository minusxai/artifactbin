...
import {saveConnection,connectionConfig} from '@/lib/datasets/connections';
...
async function makePostgres(id:string){const db=await harness.db();const row=(await db.query<{meta:Record<string,unknown>}>('SELECT meta FROM artifacts WHERE id=$1',[id])).rows[0];await db.query('UPDATE artifacts SET meta=$2::jsonb WHERE id=$1',[id,JSON.stringify({...row.meta,catalog:{kind:'postgres',connectionId:'conn_test',defaultSchema:'public',refreshSeconds:60,tables:[{schema:'public',name:'rows',columns:[{name:'n',type:'number'}],source:{schema:'public',table:'rows'}}]}})]);}
...
describe('Postgres dataset access boundaries',()=>{
 it('refuses readwrite before applying any other sharing fields and reports dataset kind',async()=>{
  const {token,id}=await stored();await makePostgres(id);const cookie=await agentCookie([token.id]);
  ...
  expect(await rawResponse.json()).toMatchObject({catalog:{kind:'postgres',connectionId:'conn_test'}});
 });
 ...
 it('a connection created by a token follows it into the claimed account scope',async()=>{
  const token=await mintToken('connection');const actor={tokenId:token.id,userId:null};
  const saved=await saveConnection(actor,{name:'db',host:'db.example',port:5432,database:'app',username:'reader',password:'secret',ssl:true});
  const user=await createUser({email:'mxmx_test_connection_claim@example.com'});await claimToken(user.id,token.token);
  await expect(connectionConfig(saved.id,{tokenId:'',userId:user.id})).resolves.toMatchObject({host:'db.example',password:'secret'});
  const stranger=await createUser({email:'mxmx_test_connection_stranger@example.com'});
  await expect(connectionConfig(saved.id,{tokenId:'',userId:stranger.id})).rejects.toThrow(/not found/i);
 });
});
