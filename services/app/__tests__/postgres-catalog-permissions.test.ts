...
it.each(['bearer','browser'] as const)('returns a controlled unavailable-secret revert response through %s without changing the head',async transport=>{
 const f=await pgFixture();
 const changed=await replace(await observedRequest(`/api/artifacts/${f.id}`,{method:'PUT',token:f.ownerToken.token,json:{dataset:{...f.definition,refreshSeconds:0},expectedVersion:1}}),ctx(f.id));expect(changed.status,await changed.clone().text()).toBe(200);
