...
describe('cli-export', () => {
  ...
  it('renders explicit published URLs through viewer routes, maps --page to slide, and refuses draft HTML and historical images',async()=>{
   ...
   try{
    ...
    // An explicit URL selects the published head; local paths now use the local image renderer.
    const shot=await run(['export',`${CLI_SERVER}/a/${id}`,'--format','png','--page','2','--output','slide.png']);
...
describe('cli-fork', () => {
  ...
  it('refuses lineage naming a source the pushing account cannot read, and never overwrites a destination',async()=>{
   ...
   try{
    ...
    const draft=parseDocument(await readFile(join(root,'claim.jsx'),'utf8'));expect(draft.metadata.id).toBeTruthy();expect(draft.metadata.head_version).toBeUndefined();expect(await getArtifactById(draft.metadata.id!)).toBeNull();
