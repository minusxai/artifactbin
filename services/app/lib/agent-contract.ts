...
Not installed? \`curl -fsSL ${origin}/chat/install.sh | sh\`. Windows x64: download ${origin}/chat/install.ps1 and run it in PowerShell. Never print a credential, put one in a URL, or commit one; the CLI keeps its own, scoped to \`${origin}\`, and sends it nowhere else.
