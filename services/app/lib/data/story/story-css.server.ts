...
declare const __MX_TAILWIND_THEME_CSS__: string;
declare const __MX_TAILWIND_PREFLIGHT_CSS__: string;
declare const __MX_TAILWIND_UTILITIES_CSS__: string;

type TailwindSheetName = 'index.css' | 'theme.css' | 'preflight.css' | 'utilities.css';
const TAILWIND_SHEET_NAMES: readonly TailwindSheetName[] = ['index.css', 'theme.css', 'preflight.css', 'utilities.css'];
const embeddedTailwindSheets = typeof __MX_TAILWIND_INDEX_CSS__ === 'undefined' ? null : new Map<TailwindSheetName, string>([
  ['index.css', __MX_TAILWIND_INDEX_CSS__],
  ['theme.css', __MX_TAILWIND_THEME_CSS__],
  ['preflight.css', __MX_TAILWIND_PREFLIGHT_CSS__],
  ['utilities.css', __MX_TAILWIND_UTILITIES_CSS__],
]);
let tailwindSheets: Promise<Map<TailwindSheetName, string>> | undefined;

/** The build embeds these four data files; source-mode tests/dev read the same package files. */
function loadTailwindSheets(): Promise<Map<TailwindSheetName, string>> {
  if (embeddedTailwindSheets) return Promise.resolve(embeddedTailwindSheets);
  return tailwindSheets ??= (async () => {
    const require = createRequire(import.meta.url);
    const indexPath = require.resolve('tailwindcss/index.css');
    const base = path.dirname(indexPath);
    return new Map(await Promise.all(TAILWIND_SHEET_NAMES.map(async (name) => [name, await readFile(path.join(base, name), 'utf8')] as const)));
  })();
}
 ...
 * compile: token utilities resolve through `--card` etc. wherever the variable blocks below
 * (or a theme's `[data-theme]` block) define them.
 */
...
const SHADCN_NEUTRAL_LIGHT_BODY = `{
  --radius: 0.625rem;
...
const SHADCN_NEUTRAL_DARK_BODY = `{
...
// COLOR_PALETTE head) into every NEUTRAL token body; [data-theme] blocks still override.
 ...
 * blocks are unaffected (custom-property VALUES, not utilities), so theme
 ...
 */
// Compile memo: the read path (storyLoader) recompiles stale stories on every load, and every
// compile carries the full recipe union — identical inputs recur (hot public
 ...
 * source. `storyCssCompileVersion` hashes the recipe union + the theme emitter
...
export async function currentStoryCss(
  ...
  if (meta.compiledCss != null && meta.cssCompileVersion === storyCssCompileVersion()) {
