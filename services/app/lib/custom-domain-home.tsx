 ...
 * built stylesheet the app page links. Only the page around it is its own: the
 * head (title, description, unfurl tags, a self-canonical) and the "Made with
 * artifactbin" line a post ends with. Server-rendered once, with no script: a
...
/**
 * The page's own policy: no script at all, and nothing from anywhere but this
 * host — the app's stylesheet and its fonts, the card thumbnails and the
 * owner's picture. Inline styles stay admitted: the listing's components set
 * a few `style` attributes (an avatar's colour, a card's reveal delay).
 */
export const DOMAIN_HOME_CSP = "default-src 'none'; style-src 'self' 'unsafe-inline'; font-src 'self'; img-src 'self' data:; base-uri 'none'; form-action 'none'; frame-ancestors 'none'";
