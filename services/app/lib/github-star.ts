 ...
 * --button-star-iconColor (light #eac54f, dark #e3b341). The inherited
 * color-scheme selects the matching Primer color: https://primer.style/product/primitives/color/ */
export function githubStarMarkup(showCount = true): string {
  return `<a href="${REPO_URL}" target="_blank" rel="noopener noreferrer" aria-label="Star artifactbin on GitHub" style="display:inline-flex;align-items:center;justify-content:center;box-sizing:border-box;height:28px;overflow:hidden;border:1px solid color-mix(in srgb,currentColor 25%,transparent);border-radius:5px;background:var(--mx-reader-bg,var(--color-surface,#fff));color:inherit;text-decoration:none;font:600 12px system-ui;letter-spacing:normal;white-space:nowrap"><span style="display:inline-flex;align-items:center;gap:4px;height:100%;padding:0 8px;background:linear-gradient(transparent,color-mix(in srgb,currentColor 7%,transparent))"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="light-dark(#eac54f, #e3b341)" aria-hidden="true"><path d="m12 2.5 2.94 5.96 6.58.96-4.76 4.64 1.12 6.55L12 17.52l-5.88 3.09 1.12-6.55L2.48 9.42l6.58-.96Z"></path></svg><span>Star</span></span>${showCount ? '<span data-mx-github-count hidden style="align-self:stretch;align-content:center;padding:0 8px;border-left:1px solid color-mix(in srgb,currentColor 25%,transparent);font-variant-numeric:tabular-nums"></span>' : ''}</a>`;
}
