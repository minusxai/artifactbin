...
/** UTF-8 bytes of a string, without TextEncoder's realm differences mattering. */
const utf8 = (input: string): Uint8Array => new TextEncoder().encode(input);

export function sha256Hex(input: string): string {
  const bytes = utf8(input);
  const bitLength = bytes.length * 8;
  // Padded to a multiple of 64 bytes: 0x80, zeroes, then the length as 64 bits.
  const padded = new Uint8Array(((bytes.length + 9 + 63) >> 6) << 6);
  padded.set(bytes);
  padded[bytes.length] = 0x80;
  const view = new DataView(padded.buffer);
  // The high word of the length: safe for anything a URL could be.
