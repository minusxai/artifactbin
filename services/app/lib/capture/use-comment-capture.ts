...
export function useCommentCapture(id:string,editId:string|undefined){
 const session=useRef<CaptureSession|null>(null),generation=useRef(0),revision=useRef(editId),requestKey=useRef(crypto.randomUUID());
 ...
 const reset=useCallback(()=>{generation.current++;session.current?.dispose();session.current=null;setDraft(null);setEditing(false);setBusy(false);setRequired(false);setError('');requestKey.current=crypto.randomUUID();},[]);
 useEffect(()=>()=>{generation.current++;session.current?.dispose();},[id]);
 const start=async()=>{
 ...
 const upload=async(file:File)=>{
  if(!editId)return;setBusy(true);setError('');
  try{
 ...
 return {draft,editing,busy,required,error,requestKey,reset,start,capture,upload,stage,
  edit:()=>setEditing(true),cancelEdit:()=>setEditing(false),
  done:(preview:Blob,strokes:BrushStroke[])=>{setDraft(value=>value?{...value,preview,strokes}:null);setEditing(false);requestKey.current=crypto.randomUUID();},
  skip:()=>{reset();},
 };
}
