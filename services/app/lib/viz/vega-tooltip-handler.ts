 ...
 * so an iframe pointer's client coordinates would land on a tooltip in the top
 ...
 * up would orphan a visible, `position: fixed`, `pointer-events: none` card in the body with
 ...
 * its own. A re-scope releases the chart it replaces, so the last chart in a document is the
 * one nothing else releases: without this, its listener set and a strong reference to the
 * now-detached element outlive it. `VegaChart` calls this from the same cleanup as
 * `hideVegaTooltip`; a chart that has since handed the scope to a sibling must take nothing
 * away from it.
 */
