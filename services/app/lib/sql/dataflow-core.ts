...
export interface RunDataflowOptions {
  ...
  /** Complete authorized public.rows input for a stored source, never a remote
   * catalog/model. Used internally only for an exact whole-table source read;
   * sourceQuery still owns the bounded result returned to the reader. */
  sourceInput?: (id:string)=>Promise<DatasetTables[string] | undefined>;
...
export async function evaluateDataflow(engine:DataflowEngine,flow: Dataflow, datasets: DatasetTables, opts: RunDataflowOptions = {}): Promise<DataflowState> {
  ...
  const dependencies=new Set(localQueries.flatMap(q=>queryDeps(q.sql,queries.map(query=>query.name))));
  await Promise.all(queries.filter(q=>q.source).map(async q=>{
    try{
      ...
      // The migration emits exactly this projection. Binding the complete
      // stored input preserves legacy aggregates beyond the display cap. Do
      // not broaden this recognizer to filters, models or arbitrary SQL.
      const wholeTable=dependencies.has(q.name) && /^\s*select\s+\*\s+from\s+public\.rows\s*;?\s*$/i.test(q.sql);
      const input=wholeTable ? (opts.sourceInput ? await opts.sourceInput(q.source!) : !opts.sourceQuery ? table : undefined) : undefined;
