...
const DOCS_LISTING_HEADER = (base: string, tree: SkillTree, transport: DocTransport = 'curl') =>
  `# artifactbin docs — brief + ${tree.files.filter((f) => f.audience === 'agent' && f.ref).length} references; critical content at the TOP.
# Read ${base}/docs/${ROOT_SKILL}/${SKILL_FILE_NAME}${transportQuery(transport)} first. Lines: URL, when to read.
# Download: curl -s "${base}/docs?download=true${transport === 'mcp' ? '&transport=mcp' : ''}" | tar xz (then grep -rl <term> skills/)`;
