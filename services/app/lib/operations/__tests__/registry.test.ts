...
describe('the registry is curated, not generated', () => {
  ...
  it('carries the artifact and dataset operations of the protocol', () => {
    expect(OPERATIONS.map((o) => o.name).sort()).toEqual([
      ...
      'get_dataset_policy', 'get_version', 'list_artifacts', 'list_versions', 'mutate_dataset', 'preview_dataset_notebook', 'query_resource', 'refresh_asset', 'restore_artifact', 'revert_artifact',
      'set_dataset_policy', 'update_artifact', 'update_metadata',
    ]);
  });
  ...
  it('read/write/destructive is annotated, and the reads are the reads', () => {
    ...
    expect(readOnly).toEqual(['discover_dataset_source', 'export_artifact', 'get_artifact', 'get_dataset_policy', 'get_version', 'list_artifacts', 'list_versions', 'preview_dataset_notebook', 'query_resource']);
