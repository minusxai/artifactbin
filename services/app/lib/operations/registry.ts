...
const forkArtifactOp: Operation = {
  ...
  description: 'Copy an artifact you can READ — your own, one shared with your account, or any public/unlisted one — into a new artifact of your own at a new id and url. Use it instead of create_artifact when adapting a document that already exists: fork it, then edit the copy with edit_artifact. Content, title, theme, template and settings travel; version history, comments and shares do not (the copy is version 1, with its own edit_id). Every ref: image, dataset and recipe is re-validated AS YOU, so a document that reads a private one is refused by name instead of copied broken. Forking an APP copies the datasets its <Mutation> declarations WRITE — rows, columns, access and write policy — under your account and repoints the page at the copies; datasets it only READS keep their ref. Pass dry_run: true to be told which datasets that would copy, creating nothing. Optional title, visibility and parent_id land on the copy only — the original is never touched. Folders and live Postgres datasets are not forkable. Answers the create reply plus forked_from and datasets — the copies it made, each with its own forked_from.',
  input: {
    id: z.string(),
