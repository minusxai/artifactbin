...
export type DocumentCommitResult={applied:true;row:ArtifactRow}|{applied:false;head:ArtifactRow};
export async function commitDocumentUpdate(db:Queryable,actor:TokenActor|null,scope:Scope,id:string,update:DocumentUpdate):Promise<DocumentCommitResult|null>{
 ...
 const result=await db.query<{applied:boolean;artifact:ArtifactRow}>(`WITH RECURSIVE observed AS MATERIALIZED (
  ...
  SELECT artifacts.* FROM artifacts WHERE id=$1 AND ${scope.where('$2')}
   AND sharing_revision=(SELECT sharing_revision FROM observed) FOR UPDATE OF artifacts
  ...
  SELECT p.ancestor_ids||p.id AS ancestors FROM artifacts p,locked l WHERE ${hasParent}::boolean AND p.id=${parent}::text AND p.format='folder' AND p.deleted_at IS NULL
   ...
   AND p.id<>l.id AND NOT l.id=ANY(p.ancestor_ids) AND cardinality(p.ancestor_ids)+1<6 FOR SHARE OF p
 ), transformed AS MATERIALIZED (
  SELECT l.*,${sql.expression} AS next_document FROM locked l
  WHERE l.format='markup' AND (${replacement}::jsonb IS NOT NULL OR l.document->>'policy'=${policy}) AND ${sql.guard} AND (NOT ${whole}::boolean OR l.version=${wholeVersion}::int)
  ...
  INSERT INTO artifact_shares(artifact_id,email,role)
  SELECT u.id,x->>'email',x->>'role' FROM updated u CROSS JOIN jsonb_array_elements(${shares}::jsonb) x
  ON CONFLICT (artifact_id,email) DO UPDATE SET role=EXCLUDED.role
 ), aliases AS (
  ...
  SELECT true AS applied,to_jsonb(u)-'previous' AS artifact,u.id FROM updated u WHERE EXISTS(SELECT 1 FROM logged)
  UNION ALL SELECT false,to_jsonb(l),l.id FROM locked l WHERE NOT EXISTS(SELECT 1 FROM updated)
 ) SELECT applied,artifact||jsonb_build_object('open_annotations',(SELECT count(*) FROM annotations a WHERE a.artifact_id=response.id AND a.root_id IS NULL AND a.deleted_at IS NULL AND a.status='open'),'shares',COALESCE(CASE WHEN applied THEN ${shares}::jsonb END,(SELECT jsonb_agg(jsonb_build_object('email',s.email,'role',s.role) ORDER BY s.email) FROM artifact_shares s WHERE s.artifact_id=response.id),'[]'::jsonb)) AS artifact FROM response`,sql.params);
 const row=result.rows[0];if(!row)return null;
 const artifact=await hydrateArtifactDocument(row.artifact);
 return row.applied?{applied:true,row:artifact}:{applied:false,head:artifact};
}
