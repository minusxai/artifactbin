...
import {inertProse,proseSource,PROSE_POLICY,type ProseOperation} from './document-prose';
...
export async function commitProseOperation(db:Queryable,actor:TokenActor,id:string,baseEditId:string,op:ProseOperation,scope:Scope,shares:string):Promise<EditOutcome|null>{
 ...
 const result=await db.query<{artifact:ArtifactRow}>(`WITH observed AS MATERIALIZED (
  ...
  WHERE artifacts.id=l.id AND l.format='markup' AND l.document#>>'{prose,epoch}'=b.document_state->>'epoch'
   ...
   AND (l.document#>>'{prose,bytes}')::int+$7::int BETWEEN 0 AND $13::int
   AND l.meta->>'cssCompileVersion'=$14 AND l.document#>>'{prose,policy}'=$18
  RETURNING artifacts.*,l.shares,${start} AS text_start,l.document AS old_document,l.meta AS old_meta,l.version AS old_version,l.document_archived_at AS old_archived_at,l.actor_user_id AS old_actor_user_id,l.actor_token_id AS old_actor_token_id
 ), archived AS (
  ...
  SELECT id,edit_id,text_start,$15,$16,text_start,text_start+$17::int,$10,$11,jsonb_build_object('epoch',document#>>'{prose,epoch}','version',version) FROM updated
 ...
 [id,scope.val,key,op.path,op.newText,after.length,Buffer.byteLength(after)-Buffer.byteLength(before),baseEditId,editId,actor.userId,actor.tokenId||null,op.oldText,MAX_CONTENT_BYTES,storyCssCompileVersion(),before,after,before.length,PROSE_POLICY]);
