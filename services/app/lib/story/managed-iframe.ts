...
function safeUrl(value:string, script=false): boolean {
  ...
  return !script && (/^#[^\s]*$/.test(value) || /^ref:[A-Za-z0-9_-]+$/.test(value) || /^data:image\/(?:png|jpeg|gif|webp|avif);base64,[A-Za-z0-9+/=]+$/i.test(value));
}
...
export function compileManagedIframe(node: JsxElement): ManagedIframeContent {
  if(node.tag!=='Iframe' || node.control)fail('expected managed Iframe');
  let size=0;
  ...
  const render=(child:JsxNode,depth=0):string=>{
    ...
    if(child.isComponent || child.control || deniedTags.has(tag) || (!htmlTags.has(tag) && !['script','style','title','canvas'].includes(tag)))fail(`disallowed inner tag <${child.tag}>`);
