...
describe('the kit can show a person and ask a guest to sign in', () => {
  ...
  it('accepts them at publish', () => {
    const source = '<div data-design="tw">{$_me ? <p>Paid by <User userId="$_me" /></p> : <SignIn>Sign in to add an expense</SignIn>}</div>';
    expect(validateJsxSource(source, JSX_STORY_COMPONENT_NAMES, STORY_HTML_TAGS, 'no-inline-style')).toEqual([]);
    const halves = '<div data-design="tw"><UserImage userId="$_me" size="lg" /><UserHandle userId="$_me" /></div>';
...
describe('the publish door', () => {
  ...
  it('accepts reading the viewer and refuses binding or declaring it', () => {
    ...
    expect(refusals('<div><User userId="$_me" /></div>')).toEqual([]);
    ...
    expect(refusals('<div><UserImage userId="$_me" /><UserHandle userId="$_me" /></div>')).toEqual([]);
