...
export function validateQueryValues(flow:Pick<CompiledDataflow,'values'>,values:Record<string,Scalar>):{code:string;names:string[]}|null{
 ...
 const invalid=Object.entries(values).filter(([name,value])=>{
  if(value===null)return false;const type=declarations.get(name)!.type;if(type==='table')return true;
  return type==='timestamp'?!isTimestamp(value):type==='user'?typeof value!=='string':type==='date'?typeof value!=='string'||!Number.isFinite(Date.parse(value)):typeof value!==type;
 }).map(([name])=>name);
