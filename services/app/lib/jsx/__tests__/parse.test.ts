...
/**
 * A syntax error must say WHERE, in the caller's own coordinates.
 *
 * The parser wraps the source in `<>…</>` to allow several roots, so acorn's
 * position is two characters ahead of the source the agent actually holds —
 * and only the message string survived, so the door could report nothing but
 * "Unexpected token (1:92)". Measured on the eval matrix: opencode failed
 * publish SEVEN times in one dashboard run, re-sending the whole document each
 * time because the error gave it nothing to correct.
 */
describe('a syntax error carries its position', () => {
  it('reports the offset in the ORIGINAL source, not the wrapped one', () => {
    const src = '<div>ok</div><p>{{</p>';
    const r = parseJsx(src);
    expect(r.ok).toBe(false);
    if (r.ok) return;
    expect(r.pos).toBeGreaterThanOrEqual(src.indexOf('{{'));
    // The wrapper adds two characters; a position that includes them points at
    // the wrong character of the caller's document.
    expect(r.pos).toBeLessThan(src.length);
  });

  it('never reports a negative position, whatever acorn says', () => {
    const r = parseJsx('</>');
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.pos === undefined || r.pos >= 0).toBe(true);
  });
});
