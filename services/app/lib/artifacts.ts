 ...
 * — whatever the query typed them as. A computed column (a join's `person`, a
 * group-by's `who`) reaches DuckDB as text, so it is not a `user` column, yet
 * every id in it is a person the page will name; without this, anyone who
 * appears only there renders "Unknown person".
 */
const personColumnsDrawn = (source: string | null | undefined): Set<string> =>
  new Set([...(source ?? '').matchAll(/<User\b[^>]*\bid=["']\$_row\.([A-Za-z_][A-Za-z0-9_]*)["']/g)].map((m) => m[1]!));
