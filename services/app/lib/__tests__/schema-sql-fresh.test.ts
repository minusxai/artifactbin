...
describe('the rendered schema', () => {
  ...
  it('every table has exactly one owning package, and the declared set is the literal below', () => {
    ...
    expect(Object.keys(owners).sort()).toEqual(['app.annotations', 'app.artifact_edits', 'app.artifact_node_aliases', 'app.artifact_shares', 'app.artifact_source_ids', 'app.artifact_versions', 'app.artifacts', 'app.analytics_events', 'app.codes', 'app.dataset_secrets', 'app.dataset_result_cache', 'app.node_identity_migration_jobs', 'app.relations', 'app.tokens', 'app.users', 'app.web_assets', 'app.webfonts', 'auth.clients', 'auth.credentials', 'events.events'].sort());
