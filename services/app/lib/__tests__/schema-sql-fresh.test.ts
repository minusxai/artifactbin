...
describe('the rendered schema', () => {
  ...
  it('roles.sql creates a role per package and the schema each one OWNS, grants USAGE, and grants no table privilege', () => {
    ...
    expect(roles).toMatch(/CREATE SCHEMA auth AUTHORIZATION artifactbin_auth/);
    expect(roles).toMatch(/CREATE SCHEMA events AUTHORIZATION artifactbin_events/);
    expect(roles).toMatch(/GRANT USAGE ON SCHEMA app TO artifactbin_auth/);
  ...
  it('every table has exactly one owning package, and the declared set is the literal below', () => {
    ...
    expect(Object.keys(owners).sort()).toEqual(['app.event_outbox','events.deliveries','app.artifact_members', 'app.member_notifications', 'app.user_blocks', 'app.remote_agents', 'app.remote_work', 'app.export_image_cache', 'app.export_images', 'app.annotations', 'app.mutation_receipts', 'app.artifact_creation_operations', 'app.artifact_edits','app.artifact_id_registry','app.id_reservation_batches', 'app.artifact_node_aliases', 'app.artifact_shares', 'app.artifact_source_ids', 'app.artifact_versions', 'app.artifacts', 'app.analytics_events', 'app.browser_test_users', 'app.codes', 'app.comment_images', 'app.dataset_secrets', 'app.dataset_result_cache', 'app.dataset_policy_audit', 'app.dataset_usage', 'app.node_identity_migration_jobs', 'app.relations', 'app.tokens', 'app.users', 'app.web_assets', 'app.webfonts', 'auth.clients', 'auth.credentials', 'events.events'].sort());
    expect(new Set(Object.values(owners))).toEqual(new Set(['app', 'auth', 'events']));
  });
});
