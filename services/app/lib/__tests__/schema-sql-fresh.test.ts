...
describe('the rendered schema', () => {
  ...
  it('every table has exactly one owning package, and the declared set is the literal below', () => {
    ...
    expect(Object.keys(owners).sort()).toEqual(['app.annotations', 'app.artifact_edits', 'app.artifact_shares', 'app.artifact_versions', 'app.artifacts', 'app.analytics_events', 'app.codes', 'app.tokens', 'app.users', 'app.webfonts', 'auth.clients', 'auth.credentials'].sort());
