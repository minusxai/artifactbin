...
describe('the rendered schema', () => {
  ...
  it('every table has exactly one owning package, and the declared set is the literal below', () => {
    ...
    expect(Object.keys(owners).sort()).toEqual(['app.admin_document_audit', 'app.export_image_cache', 'app.export_images', 'app.annotations', 'app.mutation_receipts', 'app.artifact_creation_operations', 'app.artifact_edits', 'app.artifact_node_aliases', 'app.artifact_shares', 'app.artifact_source_ids', 'app.artifact_versions', 'app.artifacts', 'app.analytics_events', 'app.codes', 'app.dataset_secrets', 'app.dataset_result_cache', 'app.dataset_policy_audit', 'app.dataset_usage', 'app.node_identity_migration_jobs', 'app.relations', 'app.tokens', 'app.users', 'app.web_assets', 'app.webfonts', 'auth.clients', 'auth.credentials', 'events.events'].sort());
