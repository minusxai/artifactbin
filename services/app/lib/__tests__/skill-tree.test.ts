/**
 * The docs are ONE skill (`skills/artifact-bin/`): `SKILL.md` — the brief with
...
describe('the rules, each seen to fire on a literal tree', () => {
  it('a directory is a skill: it must hold a SKILL.md, and the root skill must exist', () => {
    ...
    expect(problems).toContainEqual(expect.stringContaining('no artifact-bin/SKILL.md'));
  });
  it('names follow the file: SKILL.md → the directory, a reference → its basename', () => {
    expect(expectedSkillName({ dir: 'artifact-bin', file: 'SKILL.md', ref: false })).toBe('artifact-bin');
    expect(expectedSkillName({ dir: 'artifact-bin', file: 'markup-data.md', ref: true })).toBe('markup-data');
    const tree = buildSkillTree({ 'artifact-bin/SKILL.md': ok('artifact-bin'), 'artifact-bin/references/markup-data.md': ok('data') });
    expect(validateSkillTree(tree, render)).toContainEqual(expect.stringContaining('artifact-bin/references/markup-data.md: name must be "markup-data"'));
  });
  it('a skill file is SKILL.md or references/<file>.md — nothing else, nothing deeper', () => {
    expect(() => buildSkillTree({ 'artifact-bin/data.md': ok('data') })).toThrow(/SKILL\.md or references/);
    expect(() => buildSkillTree({ 'artifact-bin/references/deep/x.md': ok('x') })).toThrow(/SKILL\.md or references/);
  });
  it('frontmatter: description required, ≤ 1,024 chars, third person; name [a-z0-9-] without "claude"', () => {
    const tree = buildSkillTree({
      'artifact-bin/SKILL.md': `---\nname: artifact-bin\ndescription: You can read this.\n---\n## Read first\n`,
      'claude-tips/SKILL.md': `---\nname: claude-tips\ndescription: ${'x'.repeat(1025)}\n---\n## Read first\n`,
      'markup/SKILL.md': `---\nname: markup\n---\n## Read first\n`,
    });
    const problems = validateSkillTree(tree, render);
    expect(problems).toContainEqual(expect.stringContaining('artifact-bin/SKILL.md: description must be third person'));
  ...
  it('the body opens with "## Read first", and that block is bounded', () => {
    const tree = buildSkillTree({
      'artifact-bin/SKILL.md': ok('artifact-bin', '## Intro\n\ntext\n'),
      'artifact-bin/references/markup.md': ok('markup', `## Read first\n\n${'x'.repeat(2600)}\n\n## Next\n`),
    });
    const problems = validateSkillTree(tree, render);
    expect(problems).toContainEqual(expect.stringContaining('artifact-bin/SKILL.md: the first heading must be "## Read first"'));
    expect(problems).toContainEqual(expect.stringMatching(/artifact-bin\/references\/markup.md: the Read first block is 260\d B, cap 2500/));
  ...
  it('a pattern that matches nothing is a problem, and a wrong directory does not save a name', () => {
    ...
    expect(mentionResolves(buildSkillTree({ 'artifact-bin/SKILL.md': ok('artifact-bin'), 'artifact-bin/references/themes-.md': ok('themes-') }), 'themes-<name>.md')).toBe(false);
    // A backslash cannot reach the matcher, and would not be a metacharacter if it did.
    expect(mentionResolves(buildSkillTree({ 'artifact-bin/SKILL.md': ok('artifact-bin') }), 'a\\d.md')).toBe(false);
  });
  it('links resolve one level deep: SKILL.md → its references, a reference → a sibling or its SKILL.md', () => {
    const tree = buildSkillTree({
      ...
      'artifact-bin/references/markup.md': ok('markup', '## Read first\n\n[data](markup-data.md) [up](../SKILL.md), the api at /docs/artifact-bin/SKILL.md, and [[ base ]]/docs/artifact-bin/SKILL.md is fine\n'),
      'artifact-bin/references/markup-data.md': ok('markup-data'),
    });
    const problems = validateSkillTree(tree, render);
    expect(problems).toContainEqual(expect.stringContaining('link "references/nowhere.md" → artifact-bin/references/nowhere.md does not exist'));
    ...
    expect(resolveSkillLink({ path: 'artifact-bin/references/markup.md' } as never, 'markup-data.md')).toBe('artifact-bin/references/markup-data.md');
    expect(resolveSkillLink({ path: 'artifact-bin/references/markup.md' } as never, '../SKILL.md')).toBe('artifact-bin/SKILL.md');
    expect(resolveSkillLink({ path: 'artifact-bin/SKILL.md' } as never, 'references/markup.md')).toBe('artifact-bin/references/markup.md');
  });
  it('the old delimiter is refused by name — "{{ base" would render literally', () => {
    const tree = buildSkillTree({ 'artifact-bin/SKILL.md': ok('artifact-bin', '## Read first\n\n{{ base }}/api\n') });
  ...
  it('a typo in a variable is a render failure, never an empty string', () => {
    const tree = buildSkillTree({ 'artifact-bin/SKILL.md': ok('artifact-bin', '## Read first\n\n[[ basee ]]/api\n') });
    expect(() => render(tree.files[0])).toThrow(/artifact-bin\/SKILL.md/);
  });
  it('size: ≤ 8,192 B and ≤ 500 lines rendered; over 100 lines needs a Contents; the listing is capped', () => {
    ...
    expect(problems).toContainEqual(expect.stringMatching(/artifact-bin\/SKILL.md: 90\d\d B rendered, cap 8192/));
    expect(problems).toContainEqual(expect.stringContaining('artifact-bin/references/markup.md: 124 lines and 3 sections without a "## Contents"'));
  ...
  it('a theme or template reference sees its own registry entry', () => {
    const tree = buildSkillTree({
      'artifact-bin/SKILL.md': ok('artifact-bin'),
      'artifact-bin/references/themes-industry.md': ok('themes-industry', '## Read first\n\n[[ theme.label ]] / [[ theme.fonts ]] / [[ theme.defaultMode ]]\n'),
      'artifact-bin/references/templates-deck.md': ok('templates-deck', '## Read first\n\n[[ template.beats | join(" → ") ]]\n'),
    });
    expect(render(tree.get('artifact-bin/references/themes-industry.md')!)).toContain('Industry / display Inter, body Inter, mono JetBrains Mono / light');
    expect(render(tree.get('artifact-bin/references/templates-deck.md')!)).toMatch(/Cover → /);
  });
  it('the listing is one line per file — URL, description — SKILL.md first, then references by order; human files skipped', () => {
    const tree = buildSkillTree({
      'artifact-bin/SKILL.md': ok('artifact-bin'),
      'artifact-bin/references/markup-data.md': `---\nname: markup-data\ndescription: Data.\norder: 1\n---\n## Read first\n`,
      'artifact-bin/references/markup.md': `---\nname: markup\ndescription: Markup.\norder: 0\n---\n## Read first\n`,
      'artifact-bin/references/notes.md': `---\nname: notes\ndescription: For people.\naudience: human\n---\n## Read first\n`,
    });
    const listing = docsListing(tree, BASE);
    const lines = listing.split('\n').filter((l) => l.includes('\t'));
    expect(lines.map((l) => l.split('\t')[0])).toEqual([
      `${BASE}/docs/artifact-bin/SKILL.md`,
      `${BASE}/docs/artifact-bin/references/markup.md`,
      `${BASE}/docs/artifact-bin/references/markup-data.md`,
    ]);
    expect(lines[2]).toBe(`${BASE}/docs/artifact-bin/references/markup-data.md\tData.`);
    expect(listing).toContain('download=true');

    const mcpListing = docsListing(tree, BASE, undefined, 'mcp');
    expect(mcpListing).toContain(`${BASE}/docs/artifact-bin/SKILL.md?transport=mcp`);
...
describe('the real tree (skills/) obeys every rule', () => {
  ...
  it('is ONE skill — the brief over its references, nothing else preloaded', () => {
    expect(tree.dirs.map((d) => d.name)).toEqual(['artifact-bin']);
  ...
  it('carries one reference per theme and per template, named after the registry', () => {
    for (const t of STORY_THEMES) expect(tree.get(`artifact-bin/references/themes-${t.name}.md`)?.name).toBe(`themes-${t.name}`);
    for (const t of STORY_TEMPLATES) expect(tree.get(`artifact-bin/references/templates-${t.name}.md`)?.name).toBe(`templates-${t.name}`);
  });
  ...
  it('the MCP instructions are the mcp reference\'s Read first block, rendered', () => {
    ...
    expect(text).toContain(`${BASE}/docs/artifact-bin/SKILL.md`);
...
describe('each topic is taught by exactly its owner', () => {
  ...
  const R = 'artifact-bin/references';
