...
export interface NotificationInput {id:string;artifactId:string|null;recipientId:string;senderId:string;kind:string;userId?:string;source?:string|null;once?:boolean;revision?:number}
...
export async function recordNotification(tx:Queryable,n:NotificationInput):Promise<void>{
 const result=await tx.query<{revision:number}>(`INSERT INTO member_notifications(id,artifact_id,user_id,recipient_id,sender_id,kind,source,revision)
 VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(id) ${n.once?'DO NOTHING':`DO UPDATE SET kind=EXCLUDED.kind,sender_id=EXCLUDED.sender_id,revision=${n.revision?'EXCLUDED.revision':'member_notifications.revision+1'},read_at=NULL,created_at=now()`} RETURNING revision`,[n.id,n.artifactId,n.userId??n.recipientId,n.recipientId,n.senderId,n.kind,n.source??null,n.revision??1]);
...
export async function notifyThread(tx:Queryable,artifactId:string,threadId:string,senderId:string|null,kind:string,agent=false):Promise<void>{
 if(!senderId)return;
 ...
 for(const {id} of recipients){
  ...
  await recordNotification(tx,{id:`thread:${threadId}:${id}`,artifactId,recipientId:id,senderId,kind,source:`comment:${threadId}`,revision});
 }
}
