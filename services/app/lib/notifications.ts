...
export interface NotificationInput {id:string;artifactId:string|null;recipientId:string;senderId:string;kind:string;userId?:string;source?:string|null;once?:boolean;revision?:number;firstUpdateId?:string}
...
export async function recordNotification(tx:Queryable,n:NotificationInput):Promise<void>{
 const result=await tx.query<{revision:number}>(`INSERT INTO member_notifications(id,artifact_id,user_id,recipient_id,sender_id,kind,source,revision,first_update_id)
 VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT(id) ${n.once?'DO NOTHING':`DO UPDATE SET first_update_id=CASE WHEN member_notifications.read_at IS NULL THEN coalesce(member_notifications.first_update_id,EXCLUDED.first_update_id) ELSE EXCLUDED.first_update_id END,kind=CASE WHEN EXCLUDED.kind='resolved' AND member_notifications.kind IN ('reply','reply_resolved') AND member_notifications.read_at IS NULL THEN 'reply_resolved' ELSE EXCLUDED.kind END,sender_id=EXCLUDED.sender_id,revision=${n.revision?'EXCLUDED.revision':'member_notifications.revision+1'},read_at=NULL,created_at=now()`} RETURNING revision`,[n.id,n.artifactId,n.userId??n.recipientId,n.recipientId,n.senderId,n.kind,n.source??null,n.revision??1,n.firstUpdateId??null]);
...
export async function notifyThread(tx:Queryable,artifactId:string,threadId:string,senderId:string|null,kind:string,agent=false,updateId=threadId):Promise<void>{
 if(!senderId)return;
