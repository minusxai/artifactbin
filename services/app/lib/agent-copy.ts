...
export function existingPaste(base: string, artifactId: string): string {
  ...
  return `Edit my artifact at ${artifactUrl(base, artifactId)} in place. Install afbin if needed: curl -fsSL ${origin}/chat/install.sh | sh -s -- --yes. Windows: ${origin}/chat/install.ps1 (PowerShell). Run afbin help first, then afbin auth ${artifactUrl(base, artifactId)}${serverFlag}. Approve in the browser that created this artifact; guest access is fine.${serverHint}
