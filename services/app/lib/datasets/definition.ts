...
export function parseDatasetDefinition(source: string): CatalogInput {
 ...
 const tables=children.filter(child=>child.tag==='Table').map(table=>{textFree(table);const t=attrs(table,['schema','name','sourceSchema','sourceTable','columns','modelCellId','rows','sql']);const base={schema:string(t.schema,'table schema'),name:string(t.name,'table name'),...(t.columns!==undefined?{columns:columns(t.columns)}: {})};if(t.modelCellId!==undefined)return {...base,modelCellId:string(t.modelCellId,'modelCellId')};if(t.sourceSchema!==undefined||t.sourceTable!==undefined)return {...base,source:{schema:string(t.sourceSchema,'sourceSchema'),table:string(t.sourceTable,'sourceTable')}};if(t.rows!==undefined){if(!Array.isArray(t.rows)||t.rows.some(row=>!row||typeof row!=='object'||Array.isArray(row)))throw new Error('Dataset definition: rows must be objects');return {...base,rows:t.rows as Record<string,JsonValue>[]};}if(t.sql!==undefined)return {...base,sql:string(t.sql,'table sql')};return base;});
