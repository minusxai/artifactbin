...
export function serializeDatasetDefinition(definition: CatalogInput): string {
 ...
 const notebook=definition.notebook?`<Notebook>${definition.notebook.cells.map(cell=>`<SqlCell ${prop('id',cell.id)} ${prop('name',cell.name)} ${prop('sql',cell.sql)} />`).join('')}</Notebook>`:'';
 const tables=definition.tables.map(table=>`<Table ${[prop('schema',table.schema),prop('name',table.name),...(table.source?[prop('sourceSchema',table.source.schema),prop('sourceTable',table.source.table)]:[]),...(table.modelCellId?[prop('modelCellId',table.modelCellId)]:[]),...(table.rows?[prop('rows',table.rows)]:[]),...(table.sql?[prop('sql',table.sql)]:[]),...(table.columns!==undefined?[prop('columns',table.columns)]:[])].join(' ')} />`).join('');
 return `<Dataset ${root}>${connection}${notebook}${tables}</Dataset>`;
}
