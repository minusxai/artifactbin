...
const identifierShape=z.string().min(1).refine(value=>!value.includes('\0')&&Buffer.byteLength(value,'utf8')<=63,'Identifiers must contain at most 63 UTF-8 bytes and no NUL');
const tableShape=z.object({schema:identifierShape,name:identifierShape,source:z.object({schema:identifierShape,table:identifierShape}).strict().optional(),columns:z.array(z.union([identifierShape,z.unknown().transform((raw,ctx)=>{try{return parseDatasetColumn(raw);}catch(error){ctx.addIssue({code:'custom',message:error instanceof Error?error.message:'Invalid column'});return z.NEVER;}})])).optional(),sql:z.string().min(1).max(100000).optional(),rows:z.array(z.record(z.string(),z.unknown())).optional(),modelCellId:identifierShape.optional()}).strict().superRefine((table,ctx)=>{
