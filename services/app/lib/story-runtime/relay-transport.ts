...
export function createRelayTransport(target: Window, appOrigin: string, source: Window = window, timeoutMs = 20_000, writeTimeoutMs = MUTATION_REPLY_TIMEOUT_MS): QueryTransport {
  ...
  return {
    importAsset: (url, kind, signal) => new Promise<{ url: string } | { refused: string }>((settle) => {
      ...
      signal?.addEventListener('abort', () => { const waiter = importers.get(id); if (!waiter) return; importers.delete(id); waiter.clear(); settle({ refused: 'aborted' }); }, { once: true });
      post();
    }),
    run: async (values, only, localTables) => {
      const r = await send({ values, only, ...(localTables ? { localTables } : {}) });
      return {userOptions:r.userOptions,userLabels:r.userLabels, tables: r.tables, errors: r.errors, ...(r.mutationAccess ? {mutationAccess:r.mutationAccess} : {}) };
    },
