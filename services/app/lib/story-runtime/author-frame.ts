/** Fixed trusted HTTP document. Author bytes arrive only after load over a transferred port. */
...
(()=>{
  ...
  const transfer=()=>{if(!loaded||!pendingPort||transferred)return;transferred=true;frame.contentWindow.postMessage('mx:author:init','*',[pendingPort]);pendingPort=null;};
  addEventListener('message',event=>{
    ...
    frame.onload=()=>{if(loaded){parent.postMessage('mx:author:navigated','*');frame.remove();return;}loaded=true;transfer();};
    frame.srcdoc=event.data.document;document.body.append(frame);
  });
})();
`+'</script></body></html>';
