...
describe('author script capability boundary', () => {
  it('sets a declared signal through the real store', async () => {
    ...
    expect(await bridge.request({ id: 1, op: 'set', values: { count: 7 } })).toMatchObject({ id: 1, ok: true, value: { revision: 1 } });
    expect(store.getValue('count')).toBe(7);
  });
  it.each([
    { op: 'like' }, { op: 'follow' }, { op: 'edit', source: '<p>bad</p>' },
    { op: 'fetch', url: '/api/my/artifacts' }, { op: 'set', values: { unknown: 1 } },
    { op: 'set', values: { ['__proto__']: 1 } }, { op: 'set', values: { count: 'wrong type' } },
    { op: 'set', values: { count: Infinity } }, { op: 'set', name: 'count', value: {} },
    { op: 'read', names: ['/api/my/artifacts'] }, { op: 'mutate', name: 'undeclared' },
  ...
  it.each([null, [], 'bad', { id: -1, op: 'describe' }, { id: 1.2, op: 'describe' }])('fails closed on malformed envelopes: %j', async message => {
  ...
  it('revokes the channel on dispose', async () => {
    const { store, bridge } = setup();
    bridge.dispose();
    expect(await bridge.request({ id: 4, op: 'set', values: { count: 1 } })).toMatchObject({ ok: false });
  ...
  it('refuses replays without repeating a signal write', async () => {
    const { store, bridge } = setup();
    expect(await bridge.request({ id: 1, op: 'set', values: { count: 1 } })).toMatchObject({ ok: true });
    expect(await bridge.request({ id: 1, op: 'set', values: { count: 2 } })).toMatchObject({ ok: false });
  ...
  it('bounds request bursts', async () => {
    ...
    try {
      for (let id = 1; id <= 120; id++) expect(await bridge.request({ id, op: 'describe' })).toMatchObject({ ok: true });
      expect(await bridge.request({ id: 121, op: 'describe' })).toMatchObject({ ok: false });
