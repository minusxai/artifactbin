...
import {createManagedAssetResolver,prepareManagedContent,type ManagedAssetsConfig} from './managed-assets';
...
export function ManagedIframeView({compiled,store,assets,title,height,id,className,'data-mx-ast':ast}:{compiled:ManagedIframeContent;store:DataflowStore;assets?:ManagedAssetsConfig;title?:unknown;height?:unknown;id?:string;className?:string;'data-mx-ast'?:string}) {
  ...
  useEffect(()=>{
    ...
    const target=host.current,resolver=createManagedAssetResolver(assets);
    let disposed=false,stop=()=>{};setError('');
    void prepareManagedContent(compiled,resolver,target.ownerDocument).then(prepared=>{
      if(disposed)return;
      stop=startAuthorScript('',store,target.ownerDocument,{host:target,title:label,html:prepared.html,scripts:prepared.scripts,document:managedAuthorDocument(assets?.origin),assets});
  ...
  },[content,configuration,store,label]);
