...
export async function membershipInbox(actor:RoleActor,offset=0,onlyId:string|null=null){
 ...
 const rows=(await db.query<{id:string;artifact_id:string|null;revision:number;seen_revision:number;sender_id:string;username:string|null;kind:string;source:string|null;created_at:string;read_at:string|null;title:string|null;user_id:string;status:string|null;direction:string|null}>(`SELECT n.*,u.username,a.title,m.status,m.direction FROM member_notifications n LEFT JOIN artifacts a ON a.id=n.artifact_id JOIN users u ON u.id=n.sender_id LEFT JOIN artifact_members m ON m.artifact_id=n.artifact_id AND m.user_id=n.user_id WHERE n.recipient_id=$1 AND ($2::text IS NULL OR n.id=$2) AND n.kind<>'dismissed' AND (n.artifact_id IS NULL OR (a.id IS NOT NULL AND a.deleted_at IS NULL)) AND NOT EXISTS(SELECT 1 FROM user_blocks b WHERE(b.user_id=$1 AND b.blocked_user_id=n.sender_id)OR(b.blocked_user_id=$1 AND b.user_id=n.sender_id)) ORDER BY n.created_at DESC,n.id`,[actor.userId,onlyId])).rows;
 const notifications=[];
 for(const n of rows){if(!n.artifact_id){if(n.kind==='follow')notifications.push(n);continue;}const artifact=await getArtifactById(n.artifact_id);if(artifact&&await readThrough(db,artifact,actor))notifications.push(n);}
