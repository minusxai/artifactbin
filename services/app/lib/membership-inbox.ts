...
export async function membershipInbox(actor:RoleActor){
 ...
 const rows=(await db.query<{id:string;artifact_id:string;sender_id:string;username:string|null;kind:string;source:string|null;created_at:string;read_at:string|null;title:string|null;user_id:string;status:string|null;direction:string|null}>(`SELECT n.*,u.username,a.title,m.status,m.direction FROM member_notifications n JOIN artifacts a ON a.id=n.artifact_id JOIN users u ON u.id=n.sender_id LEFT JOIN artifact_members m ON m.artifact_id=n.artifact_id AND m.user_id=n.user_id WHERE n.recipient_id=$1 AND n.kind<>'dismissed' AND a.deleted_at IS NULL AND NOT EXISTS(SELECT 1 FROM user_blocks b WHERE(b.user_id=$1 AND b.blocked_user_id=n.sender_id)OR(b.blocked_user_id=$1 AND b.user_id=n.sender_id)) ORDER BY n.created_at DESC,n.id LIMIT 100`,[actor.userId])).rows;
 const notifications=[];
