...
const schema=z.object({autoAccept:z.boolean().optional(),read:z.string().max(512).optional(),revision:z.number().int().min(1).optional(),block:z.string().max(128).optional(),unblock:z.string().max(128).optional()}).strict();
export async function GET(request:Request){const actor=actorForArtifacts(await sessionActor(request));if(!actor)return unauthorized(request);try{return json(await membershipInbox(actor,Math.max(0,Math.min(100000,Number(new URL(request.url).searchParams.get('offset'))||0))));}catch(e){if(e instanceof MembershipError)return json({error:e.message},e.status);throw e;}}
