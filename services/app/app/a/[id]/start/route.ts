...
export async function GET(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (q.get('done') === '1') {
    ...
    const parsed = await parseContentInput({ markup: source }, { loadRef: refLoaderForActor(actor), importAsset: assetImporterFor(actor.tokenId, actor.userId), resolveFont: fontResolver(), overByteQuota: byteQuotaFor(actor.tokenId) });
