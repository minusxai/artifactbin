...
it('internal export transport reaches the real public-byte middleware, not the authenticated app routes',async()=>{
 ...
 const options=await testAuthOptions({upstream:async(request)=>app.fetch(request)});
...
it('production composition bypasses identity only for manifest-listed files and still denies private documents', async () => {
 ...
 try {
  for (const upstream of [inProcess(app), overHttp(server.base, secret)]) {
   ...
   const proxy = createAuthHost(await testAuthOptions({ upstream, sessions: { resolve: session } }));
...
it('serves a scoped exported image through both app and proxy asset-origin boundaries',async()=>{
 ...
 const app=createAppServer({indexHtml:async()=>'<head></head>'}),options=await testAuthOptions({upstream:async request=>app.fetch(request)});
 options.env={...options.env,APP__PUBLIC_BASE_URL:'https://example.test',APP__ASSETS_ORIGIN:'https://assets.example.test'};
 const proxy=createAuthHost(options),url=exportAssetUrl(id,'https://example.test');
 for(const fetch of [app.fetch,proxy.fetch]){
