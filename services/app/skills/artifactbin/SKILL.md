...
- A push proves markup and reads, never a write: a page with a `<Mutation>` is done only once every write has run in a live session (`afbin help live-sessions`) on a test-user fork as that test user and as yourself; on the original, check actions `--as guest` (identity writes offer Sign in and change no data). Fix, push and fork again until a pass is clean — never hand over an untested button.
