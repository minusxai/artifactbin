...
- For a supplied artifact: `afbin pull <url-or-id> --output report.jsx`, then `afbin push report.jsx` straight back with the title and the section headings — that first push is a page the person can open — then `afbin help <template>`, then fill the sections with further pushes. For a new artifact, write the title and headings, push, and fill the same way. Share its returned URL as asked.
- Few turns: `afbin help <template>` is ONE call that prints every reference you need, not one file at a time. Push runs `afbin validate` itself and publishes any referenced CSV: no separate validate, dry-run or dataset step; a refusal names the fix. A successful push IS the verification: the head is your file. Skip pulling, diffing, exporting, screenshotting or grepping it afterwards; filling sections is not re-checking.
