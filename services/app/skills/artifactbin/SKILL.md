...
- Few turns: `afbin help <template>` (ONE call prints every reference you need, not one file at a time), then push a FIRST version within three calls of the pull — title and section headings, one line each — and fill the sections in later pushes; a person is waiting on a blank page. Push runs `afbin validate` itself and publishes any referenced CSV: no separate validate, dry-run or dataset step; a refusal names the fix. A successful push IS the verification: the head is your file. Skip pulling, diffing, exporting, screenshotting or grepping it afterwards; filling sections is not re-checking.
