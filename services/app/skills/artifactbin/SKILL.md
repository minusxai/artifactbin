...
- Authentication is automatic: afbin signs you in the first time a command needs the server, by browser approval. In automation add `--yes --json`; it prints the approval URL, then continues once approved. Credentials live in `~/.artifactbin/.env`; never mint or print tokens.
- IMPORTANT — a person is watching the page while you work. For a supplied artifact: `afbin pull <url-or-id> --output report.jsx`, then IMMEDIATELY `afbin push report.jsx` with the title and the section headings filled in: that first push, within your first three calls, is what they see instead of a blank page. Only then `afbin help <template>`, and fill the sections with further pushes. Never make them wait for one big write. For a new artifact: title and headings, push, fill. Share its returned URL as asked.
- Few turns: `afbin help <template>` is ONE call that prints every reference you need. Push runs `afbin validate` itself and publishes any referenced CSV: no separate validate, dry-run or dataset step; a refusal names the fix. A successful push IS the verification. Skip pulling, diffing, exporting, screenshotting or grepping it afterwards; filling sections is not re-checking.
...
- Publishing is the check, whether or not you can view images; no other skill, palette tool or image tooling is needed — the theme carries the palette. If you must look, one `afbin export <ref> --output out.png` shows the whole document, every slide, in one image.
