...
- Authentication is automatic: afbin signs you in when a command first needs the server — browser approval. With `--yes --json` it prints the approval URL to open, then continues once approved. When a browser is unavailable (remote/headless), use `afbin auth --email <email>` directly. After browser launch failure or timeout, ask the user for their email and use that command; ask for the emailed code, then run `afbin auth --email <email> --otp <code>` and retry the original command. Email login never opens a browser. Credentials are saved in `~/.artifactbin/hosts/<origin-id>/credentials.env`; never mint or print tokens.
