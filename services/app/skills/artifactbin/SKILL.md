...
- Dataset-backed artefacts: new stored datasets are publicly readable and allow their owner’s artefacts to mutate. Use People / `afbin invite` for participants and `afbin mention` to resolve tags; accepted membership gates data actions, never commenting. Read `afbin help apps`.
