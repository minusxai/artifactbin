...
Use For for a keyed JSX template over a declared table Value or Query. Use
...
Use `<For>` for a keyed template over a declared table Value or Query. Table
...
`each` is a single signal reference, not JavaScript. `keyBy` is required and
names a unique non-null string or finite number in every row. Numeric `1` and
...
and the item/template target. DataTable uses its existing `rowKey` property
