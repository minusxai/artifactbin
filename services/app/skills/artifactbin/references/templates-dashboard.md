...
SKELETON (adapt cols/rows to the data; 12 columns × 86px rows):
  ...
  <Helmet>
    ...
    <Query name="rev_by_week">{`select date_trunc($grain, day) period, sum(revenue) revenue from sales.rows where ($region is null or region = $region) and revenue >= $min_rev group by 1 order by 1`}</Query>
    <Query name="orders_by_week">{`select date_trunc($grain, day) period, count(*) orders from sales.rows where $region is null or region = $region group by 1 order by 1`}</Query>
    <Query name="kpi_aov">{`select sum(revenue) / count(*) aov from sales.rows where $region is null or region = $region`}</Query>
    <Query name="by_region">{`select region, sum(revenue) revenue from sales.rows group by 1 order by 2 desc`}</Query>
  </Helmet>
