    ...
    <>
      <div className="mb-8 sm:mb-10">
        ...
        <p className="mt-1.5 font-sans text-sm text-muted">
          Hand an agent the instruction below along with what artifact you want,
          and follow along.
        </p>
      </div>
    </>
...
export function HomePage() {
  ...
  const insights = usePageData<HomeInsights>("/api/page/home?part=insights", {
    enabled: !!core.data?.signedIn,
  });
  const load = useCallback(() => {
    void core.refresh(true);
    void insights.refresh(true);
  }, [core.refresh, insights.refresh]);
  const wrongAccount =
    (core.data?.signedIn && core.data.accountId !== session?.user?.id) ||
    (insights.data &&
      (!insights.data.signedIn ||
        insights.data.accountId !== session?.user?.id));
  const state = {
    core: wrongAccount ? null : core.data,
    insights: wrongAccount ? null : insights.data,
    error: wrongAccount ? new Error("Account changed") : core.error,
  ...
  if (!home && publicInitial && (!session || session.kind === "none"))
    return <Landing />;
  if (!home)
    return (
      <ShellFrame>
        <main className={`${HOME_WORKSPACE_COLUMN} mt-8 pb-24`}>
          {state.error ? (
            <div role="alert">
              <p>Could not load your workspace.</p>
              <button
                aria-label="Retry workspace"
                onClick={session ? load : reload}
              >
                Try again
              </button>
            </div>
        ...
        <ShellFrame>
          <main className={`${PAGE_COLUMN} mt-8 pb-24`}>
            <div className="mb-6 flex flex-wrap items-baseline justify-start gap-x-4 gap-y-1">
              <h1 className="font-mono text-sm tracking-[0.14em] text-fg uppercase">
                Drafts held by this browser ·{" "}
              </h1>
              <a
            ...
            <div className="reveal mt-3 rounded-[6px] border border-edge bg-surface px-4 pt-4 pb-4">
              <h2 className="mb-2.5 font-mono text-[11px] tracking-[0.14em] text-muted uppercase">
                Log in
              </h2>
              <LoginForm />
            </div>
  ...
  const empty =
    ...
    home.shared.length === 0 &&
    !state.insights?.stats?.assets;
  return (
    <ShellFrame>
      <main
        className={`${empty ? PAGE_COLUMN : HOME_WORKSPACE_COLUMN} mt-8 pb-24`}
      >
        {empty ? (
          <>
            <FirstArtifact />
        ...
        {state.error && (
          <div role="alert">
            ...
            <button aria-label="Retry workspace" onClick={load}>
              Try again
            </button>
          </div>
        )}
        {empty && (
          <div className="mb-4 flex justify-end">
            <a
              ...
              className="inline-flex items-center gap-1.5 rounded border border-edge-bright px-3 py-1.5 font-mono text-xs text-accent hover:border-accent"
            >
              <DatabasePlus aria-hidden="true" size={13} strokeWidth={1.75} />Create dataset
            </a>
          </div>
        )}
        {empty ? (
          ...
          >
            {home.artifacts.length > 0 && (
              <Shelf
                ...
                scopeParentId={null}
                rows={
                  home.artifacts.map((row) => ({
                    ...row,
                    views: state.insights?.views?.[row.id],
                    sparkline: state.insights?.sparklines[row.id] ?? null,
                  })) as never
                }
              />
            )}
        ...
        {empty && (
          <>
            {state.insights && (
              <ActivityFeed
                mine={state.insights.feed.mine}
                following={state.insights.feed.following}
              />
            )}
            <p className="mt-8">
              <a
