...
Hover between the columns, then drag to adjust their widths.
...
export function DocumentPage(){
 ...
 const [title,setTitle]=useState('Untitled document'),[busy,setBusy]=useState(false),[error,setError]=useState(''),[signIn,setSignIn]=useState(false);
 async function create(){
  if(busy)return;setBusy(true);setError('');setSignIn(false);
  try{
   const response=await fetch('/api/documents',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,document:parseDocumentMdx(example)})});
