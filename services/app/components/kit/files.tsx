 ...
 * FilesAdapter); their shape is fixed — lib/folders CHILDREN_COLUMNS — and
 * every viewer-dependent decision was already made on the server: which rows
 * exist at all, whether a row has a `thumbnail`, and whether it carries
 * `views`/`sparkline`. NOTHING here filters; a null is a null because the
 * server decided this viewer does not get it.
 *
 ...
 *  - A CAPTURE DRAWS GLYPHS ONLY. A thumbnail here is another artifact's own
 *    capture, so a card of a document holding a listing would be a capture
 *    waiting on N captures, and a private child's card is a 404 to the
...
function Files({ rows, variant = "icons", capture = false, className, ...rest }: FilesProps) {
  ...
  return (
    <section data-slot="files" className={cn("my-6", className)} {...rest}>
      <ul
        aria-label="Files"
