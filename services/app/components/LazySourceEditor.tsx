...
export default function LazySourceEditor(props: SourceEditorProps) {
  ...
  const fallback = (failed: boolean) => <div className="h-full flex flex-col bg-bg text-text">
    ...
    <textarea aria-label="Markup source" className="flex-1 min-h-0 w-full resize-none p-3 font-mono text-xs bg-bg text-text"
      spellCheck={false} value={props.value} onChange={event => props.onChange(event.target.value)}
      onFocus={event => { focus.current = { start: event.target.selectionStart, end: event.target.selectionEnd }; }}
      onSelect={event => { const input = event.currentTarget; focus.current = { start: input.selectionStart, end: input.selectionEnd }; }}
      onBlur={() => { focus.current = null; }} />
  </div>;
  return <EditorBoundary key={attempt} fallback={fallback(true)}>
    {mounted ? <Suspense fallback={fallback(false)}><RichEditor {...props} initialSelection={() => focus.current} /></Suspense> : fallback(false)}
  </EditorBoundary>;
}
