...
export default function MdxArtifactEditor({id,snapshot,compiledCss,onDone,flushRef,onLeftInsetChange}:{onLeftInsetChange?:(width:number)=>void;id:string;snapshot:DocumentSnapshot;compiledCss:string|null;onDone:()=>void;flushRef:EditorFlushRef}){
 ...
 return <div className="mdx-artifact-editor" style={{'--mdx-left-inset':`${leftInset}px`} as CSSProperties}>
  ...
  {source===null?<DocumentEditor compiledCss={css} document={preview?.document??session.document} artifactId={preview?undefined:id} editable={!preview} onChange={next=>session.update(next)}/>:<section className="mdx-source-panel"><textarea aria-label="MDX source" value={source} onChange={e=>setSource(e.target.value)}/><button onClick={()=>switchMode('design')}>Apply MDX</button></section>}
 </div>;
}
