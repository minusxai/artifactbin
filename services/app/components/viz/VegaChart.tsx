...
// use child_width/child_height instead (computed in lib/viz/facet-layout). Axes/legends draw in
...
export function VegaChart({ envelope, rows, colorMode, onViewChange }: VegaChartProps) {
   ...
   * re-parsed on every edit, so identity changes whenever anything anywhere in
   * that document changes; keyed on it, an untouched chart tore itself down and
   * re-animated while the reader watched someone fix a typo elsewhere.
   */
  const specKey = specSignature(envelope);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const resolved = resolveEnvelopeSpec(envelopeRef.current, inferVizColumnsFromRows(rowsRef.current));
        ...
        // Token-form colors (`var(--foreground)`, a recipe's trendColor)
        // resolve here, per build from THIS container's computed style — the
        // same cadence as the token range above, so they re-ink on a theme or
        // mode flip while a raw hex stays deliberately pinned.
        {
        ...
        // recipe OR detached), and only when the URL is still a default — including the
        // retired absolute Carto URLs stored specs carry — a custom `tileUrl` the
        // user/agent set is left untouched (themeTileUrl, pure).
        if (hasSignal(view, 'tileUrlTemplate')) {
          const swap = themeTileUrl(view.signal('tileUrlTemplate'), colorMode);
          if (swap) view.signal('tileUrlTemplate', swap);
        }
        if (cancelled) return;
        await view.runAsync();
        ...
        if (isInteractiveMap(envelopeRef.current) && hasSignal(view, 'mxViewParams')) {
  ...
  }, [specKey, colorMode, legendEpoch, themeEpoch, replanLegendWrap]);
