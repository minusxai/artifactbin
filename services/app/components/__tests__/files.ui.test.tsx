/**
 * `<Files>` — the kit listing a DOCUMENT binds (`<Files data="$q" />` over a
 * `ref_<folderId>` query). A folder's own page is app chrome and does not come
 * through here, so what is left is the component's three rules: the card where
 * a thumbnail exists, a format glyph where it does not, and numbers only when
...
describe('<Files>', () => {
  ...
  it('shows the view count and a sparkline only where views is a number', () => {
    ...
    // A mark per row is the point, so this collects them all rather than
    // asking for one: every row the server counted gets one, and no row it
    // did not.
