...
import { pagedQuery, unwrapPaged } from './paging';
...
import { isQueryFailure,type QueryFailure } from '@artifactbin/contracts';
 ...
 */
export async function runMutation(input: MutationInput, caps: SqlCaps, extensions:SqlExtensions = {}): Promise<MutationOutcome> {
  const { limit, timeoutMs } = queryBounds(input, caps);
  const instance = await createInstance();
  const conn = await instance.connect();
  let timedOut = false;
...
export async function dryRunMutations(input: DryRunMutationsInput, extensions:SqlExtensions = {}): Promise<DryRunMutationsResult> {
  ...
  for (const m of input.mutations) {
    ...
    const conn = await instance.connect();
    try {
      extensions.setupMutation?.(conn,await duckdb(),{dryRun:true});
