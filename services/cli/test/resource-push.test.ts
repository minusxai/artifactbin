...
test('YAML dataset push publishes content and access together, tracks source bytes and skips unchanged work offline',async()=>{
 ...
 try{
  await saveTestConnection({server:'https://example.com',token:'test'},root);await seedIdentityPool(root,root,['data123'],'account');
...
test('YAML metadata push reconciles an unrelated remote field before its conditional write',async()=>{
 ...
 try{
  await saveTestConnection({server:'https://example.com',token:'test'},root);await seedIdentityPool(root,root,['data123'],'account');await writeFile(join(root,'sales.csv'),'score\n42\n');await writeFile(join(root,'sales.yaml'),'type: dataset\nsource: ./sales.csv\ntitle: Before\n');
