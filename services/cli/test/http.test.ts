...
test('ordinary writes use one conditional request, and actual 401 refreshes once without a validation GET',async()=>{
 ...
 const request:typeof fetch=async(input,init)=>{
  ...
  // Every product request names this build, so a recording proxy can tell THIS afbin from curl or another release.
  if(path.startsWith('/api/'))assert.equal(new Headers(init?.headers).get('User-Agent'),`afbin/${CLI_VERSION}`);
...
test('refresh honors the process ARTIFACTBIN_HOME when no env override is supplied',async()=>{
 const home=await mkdtemp(join(tmpdir(),'afbin-refresh-home-'));
 const previous=process.env.ARTIFACTBIN_HOME;
 const root=join(home,'custom');process.env.ARTIFACTBIN_HOME=root;
 const connection={server:'https://example.com',token:'mx_old',refreshToken:'mxr_old',clientId:'afbin_client'};
 try{
  const client=new HttpClient({connection,home,fetch:async(input,init)=>{
   if(new URL(String(input)).pathname==='/oauth/token')return Response.json({access_token:'mx_new',refresh_token:'mxr_new',expires_in:1000});
   return new Headers(init?.headers).get('Authorization')==='Bearer mx_old'
    ?Response.json({error:'unauthorized'},{status:401}):Response.json({ok:true});
  }});
  await client.request('/artifacts');
  assert.equal((await loadConnection(connection.server,home,{ARTIFACTBIN_HOME:root}))?.token,'mx_new');
  assert.equal(await loadConnection(connection.server,home,{}),null,'must not write into the default home');
 }finally{
