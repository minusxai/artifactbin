import {setClientDefault} from '../src/config';
...
test('a saved updates=false suppresses both scheduling and an already launched worker',async()=>{
 const home=await mkdtemp(join(tmpdir(),'afbin-auto-default-'));let launches=0,checks=0;
 try{
  await setClientDefault('updates','false',home,{});
  const options={home,env:{},server:'https://artifactbin.dev',standalone:true,
   launch:()=>{launches++;},update:async()=>{checks++;}};
