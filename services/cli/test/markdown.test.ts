...
/** Every case owns a private state directory: no test may read or write the developer's ~/.artifactbin. */
const isolated=()=>mkdtemp(join(tmpdir(),'afbin-markdown-home-'));
...
test('conversion plans and dry-run stay in memory; committed JSX becomes the sole editable source',async()=>{
 const root=await mkdtemp(join(tmpdir(),'afbin-markdown-'));const home=await isolated();
 try{
  await writeFile(join(root,'report.md'),'# Report\n\nKeep this original.');const workspace=await loadWorkspace(root,home);
  ...
  await assert.rejects(prepareMarkdown(await loadWorkspace(root,home),['report.md']),/report.jsx/);
  await rm(join(root,'report.jsx'));await assert.rejects(prepareMarkdown(await loadWorkspace(root,home),['report.md']),/already converted/i);
...
test('conversion refuses an existing JSX destination and source changes during planning',async()=>{
 const root=await mkdtemp(join(tmpdir(),'afbin-markdown-conflict-'));const home=await isolated();
 try{
  await writeFile(join(root,'doc.md'),'# A');await writeFile(join(root,'doc.jsx'),'<p>Keep</p>');
  await assert.rejects(prepareMarkdown(await loadWorkspace(root,home),['doc.md']),/exists/i);assert.equal(await readFile(join(root,'doc.jsx'),'utf8'),'<p>Keep</p>');
  await rm(join(root,'doc.jsx'));const plan=await prepareMarkdown(await loadWorkspace(root,home),['doc.md']);await writeFile(join(root,'doc.md'),'# B');
...
test('CLI push imports once, while dry-run only sends preflight and leaves no local state',async()=>{
 const root=await mkdtemp(join(tmpdir(),'afbin-markdown-dispatch-'));const home=await isolated();
 try{
  ...
  const context={home,cwd:root,env:{ARTIFACTBIN_TOKEN:'test_token'},interactive:false,stdout:(s:string)=>output.push(s),stderr:()=>{},fetch:async(input:unknown,init?:RequestInit)=>{
   ...
   // The first command in a fresh private state directory asks the selected origin which addresses
   // it answers at (services/cli/src/server-identity); the answer is cached for the pushes below.
   if(path==='/api/server')return Response.json({origin:'https://app.artifactbin.dev',aliases:[]});
  ...
  assert.equal(await runCli(['push','draft.md','--dry-run','--json'],context),0,output.join(''));assert.deepEqual(calls,['/api/server','/api/artifacts/preflight']);
...
test('a markdown conversion record lives in the state store and the workspace gains no .artifactbin directory',async()=>{
 ...
 try{
  ...
  await commitMarkdown(await prepareMarkdown(await loadWorkspace(root,home),['report.md']));
  const state=await State.open(home);
  try{
   const scope=await realpath(root);
   assert.deepEqual(state.get(scope,'conversion','report.md')?.value,{target:'report.jsx',sha256:digest(source)});
   assert.deepEqual(state.list(scope,'conversion').map(record=>record.key),['report.md']);
  }finally{state.close();}
  // Nothing of ours is left in the workspace: no record file, no journal, no directory to commit.
  assert.deepEqual((await readdir(root)).sort(),['report.jsx','report.md']);
  await assert.rejects(prepareMarkdown(await loadWorkspace(root,home),['report.md']),/already converted/i);
 }finally{await rm(root,{recursive:true,force:true});await rm(home,{recursive:true,force:true});}
});
