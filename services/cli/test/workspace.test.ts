...
test('push writes no afbin.lock and no .artifactbin directory; tracking lives in the state store with a hash, not bytes',async()=>{
 ...
 const {runCli}=await import('../src/dispatch');const {State}=await import('../src/state');const {digest}=await import('../src/files');
 ...
 const fetchStub:typeof fetch=async(input,init)=>{const path=new URL(String(input)).pathname;const body=JSON.parse(String(init?.body??'{}'));
  if(path==='/api/artifacts/reservations')return Response.json({ids:['abc123',...Array.from({length:99},(_,i)=>'W'+String(i).padStart(5,'0'))]},{headers:{'X-Artifactbin-Account':'usr_one'}});
 ...
 await saveTestConnection({server:'https://example.com',token:'mx_test'},home);await writeFile(join(cwd,'doc.jsx'),'<p>One</p>');
...
test('a tracked binary asset records only its sha256 and status detects a changed image without any stored copy',()=>fixture(async(home,cwd)=>{
 const fetchStub:typeof fetch=async(input,init)=>{const path=new URL(String(input)).pathname;
  if(path==='/api/artifacts/reservations')return Response.json({ids:['img123',...Array.from({length:99},(_,i)=>'W'+String(i).padStart(5,'0'))]},{headers:{'X-Artifactbin-Account':'usr_one'}});
