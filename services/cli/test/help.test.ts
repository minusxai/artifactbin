...
test('afbin help --for <template> prints every reference a document of that kind needs, in one call, in reading order',async()=>{
 ...
 try{
  ...
  assert.ok(text.includes('IMPORTANT: a person is watching the page — your first push (title and section headings) must already be live; if not, push NOW'),'the intro demands the first push before the reading, with the reason');
