...
for(const saved of [false,true])for(const explicit of [false,true]){
 test(`setup uses the selected origin without credentials (saved=${saved}, flag=${explicit})`,async()=>{
  ...
  try{
   ...
   const code=await runCli(['setup','--yes','--json','--harness','none',...(explicit?['--server',selected]:[])],{
   ...
   assert.equal(JSON.parse(output.join('')).error.verification_url,selected+'/oauth/device?user_code=ABCD-EFGH');
