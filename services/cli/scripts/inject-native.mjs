/** Build-only writer: modern LIEF preserves Intel TLS and Linux native symbol tables. */
import {existsSync} from 'node:fs';
import {resolve,join} from 'node:path';
import {fileURLToPath} from 'node:url';
import {execFileSync} from 'node:child_process';
export function injectNative(binary,blob){
 ...
 execFileSync(python,[fileURLToPath(new URL(process.platform==='linux'?'./sea-inject-elf.py':'./sea-inject-macho.py',import.meta.url)),binary,blob],{stdio:'inherit'});
}
