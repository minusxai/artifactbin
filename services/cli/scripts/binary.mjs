...
await writeFile(`${binary}.sizes.json`,JSON.stringify({platform:process.platform,arch:process.arch,node:execFileSync(runtime,['--version'],{encoding:'utf8'}).trim(),smallIcu:execFileSync(runtime,['-p','String(process.config.variables.icu_small)'],{encoding:'utf8'}).trim(),coreInstalled:rawBytes.length,coreDownload:compressed.length,components:{nodeAndContainer:rawBytes.length-javascriptBytes-terminalArchiveBytes-sqlManifestBytes,javascript:javascriptBytes,terminalArchive:terminalArchiveBytes,sqlManifest:sqlManifestBytes},sqlInstalled:duckdb.manifest.files.reduce((sum,file)=>sum+file.size,0),sqlDownload:(await readFile(join('dist',duckdb.file))).length},null,2)+'\n');
