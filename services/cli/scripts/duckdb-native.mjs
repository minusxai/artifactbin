...
export {standaloneSqlSource} from './sql-native-plugin.mjs';
import {standaloneSqlPlugin} from './sql-native-plugin.mjs';
...
export async function duckdbNative(){
 ...
 try{for(const name of [target])await collect(dirname(require.resolve(`${name}/package.json`)),'',name);}
 ...
 await build({entryPoints:[require.resolve('@duckdb/node-api')],outfile:resolve('dist/duckdb-api.cjs'),bundle:true,platform:'node',format:'cjs',target:'node22',external:['@duckdb/node-bindings']});
 return {asset,file,manifest,driver:resolve('dist/duckdb-api.cjs'),plugin:standaloneSqlPlugin()};
}
