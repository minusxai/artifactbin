...
export async function downloadRuntime(pin,{root,fetch:fetcher=fetch}) {
 if(!pin||!hash.test(pin.sha256)||!hash.test(pin.gzipSha256)||!Number.isSafeInteger(pin.size)||pin.size<1||pin.size>limit)throw new Error('Invalid prebuilt runtime pin.');
 const url=new URL(pin.url);
 if(url.origin!=='https://github.com'||!url.pathname.startsWith('/minusxai/artifactbin/releases/download/')||url.username||url.password||url.search||url.hash)throw new Error('Invalid prebuilt runtime URL.');
 ...
 if(digest(compressed)!==pin.gzipSha256)throw new Error('Prebuilt runtime transport checksum mismatch.');
 const bytes=gunzipSync(compressed,{maxOutputLength:pin.size});
