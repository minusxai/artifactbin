...
export function runtimePin(lock,platform,arch){
 ...
 if(entry.recipe?.platform!==platform||entry.recipe?.arch!==arch||entry.recipe?.version!==lock.version||entry.recipe?.intl!=='small-icu')throw new Error('Prebuilt runtime recipe does not match its pin.');
