...
try{
 ...
 assert.ok(sizes.coreDownload<35_000_000,'Thin CLI download must remain below 35 MB on every platform');
 assert.ok(sizes.runtimeDownload>0&&sizes.runtimeInstalled>sizes.runtimeDownload);
 const host=JSON.parse(await readFile('dist/runtime.manifest.json','utf8'));
