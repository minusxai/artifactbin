...
Before writing, read design, markup, then the selected template and theme guidance below. Template frames prevent content sitting flush to the viewport edge. Use container prefixes: `grid-cols-1 @2xl:grid-cols-2` and `text-3xl @2xl:text-5xl` start at phone width. For dataviz, read `references/markup-data.md` first; use Vega/Vega-Lite or recipes, never a hand-rolled `<svg>` chart. Only export screenshots if you can view images; otherwise check markup.
