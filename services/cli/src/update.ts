...
async function detectInstallation(server:string,executable=process.execPath,standalone=isSea()):Promise<Installation>{
 ...
 throw new CliError('unmanaged_installation','This afbin was not installed as a verified standalone executable.',`Install it with ${releaseOrigin(server)}/chat/install.sh, then rerun afbin update.`);
}
...
async function releasePointerOf(options:UpdateOptions,fetcher:typeof fetch):Promise<ReleasePointer>{
 const value=JSON.parse((await download(`${releaseOrigin(options.server)}${releasePointer}`,fetcher,65536,options.stallMs)).toString());
...
async function updateLocked(options:UpdateOptions){
 ...
 const commit=async()=>{
  ...
  const installed=options.background?{installations:[],harnesses:[]}:await installSkills(operation.selected,{home:options.home,env:options.env,version:bundle.version,files:bundle.files,origin:normalizeHost(options.server),alreadyLocked:true});
  await rm(pendingPath);await rm(binaryPath,{force:true});
