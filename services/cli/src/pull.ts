...
export async function pull(workspace:Workspace,args:string[],client:HttpClient,options:{format?:string;output?:string;force?:boolean;dryRun?:boolean;type?:string}={}){
 ...
 const run=async()=>{
  ...
  for(const target of targets){
   ...
   operations.push({path,...(backup?{backup}:{}),...(sourceBackups.length?{source_backups:sourceBackups}:{}),id:head.id,version:snapshot.version,head_version:head.version,status:'pulled',...(starter?{next:'IMPORTANT: a person is watching this page. Push it straight back NOW with the title and the section headings filled in — that first push is what they see instead of a blank page — then afbin help <template> (dashboard, deck, editorial, plan, scrolly) prints everything you need in one call; fill the sections with further pushes. Never make them wait for one big write.'}:{})});
