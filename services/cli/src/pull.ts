...
export async function pull(workspace:Workspace,args:string[],client:HttpClient,options:{format?:string;output?:string;force?:boolean;dryRun?:boolean;type?:string}={}){
 ...
 const run=async()=>{
  ...
  for(const target of targets){
   ...
   operations.push({path,...(backup?{backup}:{}),...(sourceBackups.length?{source_backups:sourceBackups}:{}),id:head.id,version:snapshot.version,head_version:head.version,status:'pulled',...(starter?{next:'A starter: push it straight back with the title and the section headings — that first push is a page the person can open — then afbin help <template> (dashboard, deck, editorial, plan, scrolly) prints everything it needs in one call; fill the sections with further pushes.'}:{})});
