...
export async function localAccountCommand(workspace:Workspace,parsed:ParsedCommand,plan:AccountPlan):Promise<unknown|undefined>{
 ...
 if(parsed.command==='push'&&!await pendingRecovery(workspace)&&files.every(file=>file.status==='unchanged')){
  const artifacts=await finishLocalPush(workspace,plan.artifacts,{force:!!parsed.flags.force,access:parsed.flags.access as 'read'|'readwrite'|undefined,policy:parsed.flags.policy as 'viewers-write'|'none'|undefined});
...
export async function remoteAccountCommand(workspace:Workspace,parsed:ParsedCommand,plan:AccountPlan,client:HttpClient):Promise<{value?:unknown;content?:string;exitCode?:number}>{
 ...
 if(plan.artifacts.length){
  const result=await push(workspace,plan.artifacts,client,{force:!!parsed.flags.force,dryRun:!!parsed.flags['dry-run'],access:parsed.flags.access as 'read'|'readwrite'|undefined,policy:parsed.flags.policy as 'viewers-write'|'none'|undefined});
