...
export async function deviceAuthenticate(origin: string, options: AuthOptions): Promise<Connection> {
  ...
  if (raw) {
    ...
    if (validPending(value, server, [server, ...(options.aliases ?? [])]) && value.expiresAt > clock()) pending = value;
  }
  ...
  while (clock() < deadline) {
    ...
    if (response.ok) {
      ...
      await saveConnection(connection,home,{ARTIFACTBIN_HOME:configDir(home,options.env)});
      await unlink(file);
      return connection;
    }
