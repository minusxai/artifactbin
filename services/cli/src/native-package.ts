...
function validate(spec:NativePackage,kind='sql',platform=process.platform):number{
 ...
 for(const file of spec.files){
  ...
  if(platform==='win32'&&(seen.has(key)||file.path.split('/').some(p=>/[<>:"|?*\x00-\x1f]/.test(p)||/[. ]$/.test(p)||/^(con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(p))))throw new CliError('invalid_release','Invalid Windows native package manifest path.');
...
async function verifyCache(root:string,spec:NativePackage,platform=process.platform):Promise<boolean>{
 ...
 try{
  for(const file of spec.files){
   ...
   if(file.link!==undefined){
    ...
    const target=relative(await realpath(root),await realpath(path)).split('\\').join('/');
    if(!target.startsWith('node_modules/'))throw new Error('link escape');
   }else if(!info.isFile()||info.size!==file.size||(platform!=='win32'&&(info.mode&0o777)!==(file.mode??0o600))||digest(await readFile(path))!==file.sha256)throw new Error('checksum');
  }
...
export async function ensureNativePackage(spec:NativePackage,options:{root:string;fetch?:typeof fetch;kind?:'sql'|'runtime'|'chromium';platform?:NodeJS.Platform}):Promise<string>{
 const platform=options.platform??process.platform;
 const total=validate(spec,options.kind,platform),destination=join(options.root,spec.sha256);
 if(await verifyCache(destination,spec,platform))return destination;
 ...
 try{
  offset=0;
  for(const file of spec.files){const path=join(staging,file.path);await mkdir(dirname(path),{recursive:true,mode:0o700});if(file.link===undefined)await writeFile(path,bytes.subarray(offset,offset+file.size),{mode:file.mode??0o600,flag:'wx'});offset+=file.size;}
  for(const file of spec.files)if(file.link!==undefined)await symlink(file.link,join(staging,file.path));
  await verifyCache(staging,spec,platform);
  try{await rename(staging,destination);}
  catch(error){if(!['EEXIST','ENOTEMPTY',...(platform==='win32'?['EPERM']:[])].includes((error as NodeJS.ErrnoException).code??''))throw error;}
  if(!await verifyCache(destination,spec,platform))throw new CliError('invalid_service_cache','Service cache disappeared during installation.');
  return destination;
