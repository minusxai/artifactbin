...
export async function browserSessionCommand(client: HttpClient, op: string, target: string, options: {
  code?: string; execution?: string; progress(text: string): void;
  ...
  let result = await client.request<BrowserSessionResult>('/browser-sessions', 'POST', { op, session_id, execution_id, create: target === 'new', code: options.code });
