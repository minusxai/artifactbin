import {BACKGROUND_UPDATE_ARG,TEAM_HOST_ARG,PREVIEW_HOST_ARG,LOCAL_IMAGE_ARG,REMOTE_WORKER_ARG} from './entry-args';
...
else if(process.argv[2]==='--internal-browser-worker')void import('./browser-worker-entry').then(({startBrowserWorker})=>startBrowserWorker()).catch(error=>{console.error(error);process.exit(1);});
