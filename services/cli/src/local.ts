...
export async function localDiff(workspace:Workspace,paths?:string[]){
 ...
 return{remote:'last_observed',diffs:files.filter(file=>file.status!=='unchanged').map(file=>{
  ...
  return{path:file.path,diff:file.document||file.resource?createTwoFilesPatch(`base/${file.path}`,`local/${file.path}`,before,after,'last observed','working file',{context:3}):`Binary file ${file.path}: ${file.status}`};
 })};
}
