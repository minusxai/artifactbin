...
export function remoteRequestInput(data:string,executable:string):string{
 ...
 return JSON.stringify({...payload,cli_executable:executable,instruction:`Use the absolute cli_executable for every afbin command; do not rely on PATH. ${String(payload.instruction??'Read the artifact and thread, acknowledge before work, verify and reply with the correlated request phase. Resolve only if fully addressed.')} ${REMOTE_EDITING_INSTRUCTIONS} ${COMMENT_IMAGE_REMINDER}`})+'\r';
}
