...
export function parseSessionResource(input:unknown):SessionResource{
 ...
 if(id!==undefined&&(typeof id!=='string'||!SESSION_ID.test(id)))throw new CliError('invalid_resource','Invalid session id.');
 const value=parseAccountResource({...rest,type:'session'});
 if(value.type!=='session')throw new CliError('invalid_resource','Expected a session resource.');
 return id===undefined?value:{...value,id:id as string};
}
