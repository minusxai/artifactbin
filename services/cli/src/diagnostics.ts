...
export const diagnosticCatalog:Record<string,{meaning:string;fix:string}>={
 ...
 renderer_unavailable:{meaning:'This representation requires a published head, and this target is not one.',fix:'Use PNG/JPG for a local JSX image, push before exporting the HTML offline file, or export csv, json, yaml or original.'},
 unsupported_version_export:{meaning:'This target has no version to show: the published viewer opens the head, and a local file is a draft rather than a history.',fix:'Export a published id at a version (afbin export <id>@<n> --format png), open the head, or afbin pull <ref>@<n> for that content.'},
 ...
 browser_unavailable:{meaning:'The browser could not be opened; authentication stopped immediately.',fix:'Ask for the user email and run afbin auth --email <email>, then complete with --otp <code>.'},
 otp_required:{meaning:'An email login code was sent; authentication is not complete.',fix:'Ask for the emailed code and run afbin auth --email <email> --otp <code>, using the same --server as before.'},
 otp_send_failed:{meaning:'The server could not send the login email.',fix:'Check the email and server, then retry afbin auth --email <email>; wait before retrying if rate limited.'},
 otp_rejected:{meaning:'The email code was invalid, expired, or already used.',fix:'Check the email and code, or request a new one with afbin auth --email <email>, then complete with --otp <code>.'},
 ...
 wrong_server:{meaning:'The artifact URL or workspace belongs to a different server, and that origin is not one the selected server publishes as another address of itself. The refusal names both origins: the one that was selected and the one the URL or the workspace carries.',fix:'Pass --server <the origin named in the refusal> to select that server, or run the command from a workspace tracked against it; keep work for another server in a separate directory. When one deployment answers at two hostnames, its operator publishes the second one and afbin then treats them as one server.'},
 ...
 duplicate_identity:{meaning:'Two local files claim the same published artifact.',fix:'Keep one tracked file. To READ another version without a second working copy: afbin pull <id>@<v> --output - , or afbin export <id>@<v> --format original --output <file>. To keep working on one: afbin fork <id>@<v>, which gives the copy its own identity.'},
 /*
  ...
  */
 invalid_policy:{meaning:'The dataset policy is not one this server accepts: an unsupported field, a role other than viewer, or a table or column the dataset does not have.',fix:'The detail names the field, table or column. Read the dataset\'s own tables with afbin pull <ref> --format yaml, correct the policy there and push the YAML; afbin push <ref> --policy viewers-write writes a grant for every table the dataset declares.'},
 not_found:{meaning:'The selected server does not have this artifact, or this account cannot reach it — existence is never revealed, so the two are one answer.',fix:'Check the reference with afbin list and the account with afbin auth. Someone else\'s artifact becomes yours to push only through afbin fork <ref>.'},
 policy_locked:{meaning:'The dataset carries a write policy. Replacing the content of a governed dataset is the OWNER\'s alone, and reverting one is refused for everyone, because viewers\' rows are in it.',fix:'Ask the owner to push the change, or take your own copy with afbin fork <ref>. As the owner, push the rows you want — read an older version with afbin pull <ref>@<v> --output - and push that content instead of reverting.'},
 policy_mismatch:{meaning:'The policy does not fit the dataset it is aimed at: a policy declared in the YAML disagrees with --policy, or a replacement no longer has a table or column the policy grants.',fix:'The detail names the table or column. Keep it in the pushed rows, or change the policy first: afbin pull <ref> --format yaml, edit policy there and push the YAML before replacing the content.'},
 combined_policy_write:{meaning:'One request cannot both replace content and change the policy, and a dataset that does not exist yet cannot be given one.',fix:'Push the content first, then afbin pull <ref> --format yaml and push that YAML with its policy. afbin push <ref> --policy viewers-write does both, in that order, in one command.'},
 policy_changed:{meaning:'The policy revision moved since the one this push observed, so the compare-and-swap wrote nothing.',fix:'Run afbin pull <ref> --format yaml for the current policy and its policy_revision, reapply your change there and push the YAML again.'},
 ...
 invalid_dataset:{meaning:'A local dataset is not supported tabular data.',fix:'Use CSV, a JSON array of row objects, or a .geojson FeatureCollection. Use afbin help data for Query and Mutation examples.'},
 unsupported_file_type:{meaning:'This local format cannot be published through native push.',fix:'Use a .jsx document, CSV/JSON/GeoJSON rows, or an allowed media/file format. See afbin help publishing-datasets.'},
};
