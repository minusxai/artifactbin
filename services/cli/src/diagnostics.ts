...
export const diagnosticCatalog:Record<string,{meaning:string;fix:string}>={
 ...
 image_fetch_failed:{meaning:'The server could not fetch an external image URL named by the document.',fix:'Check the URL is public and reachable, or save the image beside the document and reference its relative path; afbin push publishes it with the document.'},
 ...
 transport_error:{meaning:'The selected server could not be reached, so no request was answered.',fix:'Check the server URL (--server or ARTIFACTBIN_URL) and your network, then retry; afbin auth --server <url> changes the default server.'},
