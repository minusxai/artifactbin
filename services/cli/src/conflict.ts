...
export async function describeConflict(error:CliError,pending:PendingRequest,client:HttpClient,home:string,root:string):Promise<CliError>{
 ...
 const update=pending.request.body.document_update as import('@artifactbin/contracts').DocumentUpdate|undefined;
 const source=pending.request.body.source??pending.request.body.markup??(update?parseDocument(Buffer.from(pending.file.bytes,'base64').toString()).body:undefined);
 ...
 const metadata=Object.fromEntries(Object.entries(update?{...update.metadata,...update.settings}:pending.request.body).filter(([key])=>['title','description','theme','template','visibility','linkRole','parent_id'].includes(key)).map(([key,proposed])=>[key,{current:head[names[key]??key],proposed}]));
 const result=new CliError(error.code,error.message,'Review the conflict and preserve both writers. Edit the local file and retry, or use pull --force only after saving your proposal.',{...details,head,...(typeof source==='string'?{local:source}:{}),...(typeof remote==='string'?{remote}:{}),...(diff!==undefined?{diff}:{}),...(Object.keys(metadata).length?{metadata}:{})},error.exitCode);
