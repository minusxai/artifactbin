...
it.each(['delete','expire','unchanged'] as const)('revalidates a real database-backed session (%s) and replaces response headers safely',async(change)=>{
 ...
 try {
  ...
  const proxy=createAuthHost({env:{},cookieSecret:'test',tokens:{byToken:async()=>null,byId:async()=>null,invalidate:()=>{}},sessions:auth.sessions,upstream:async()=>{started.resolve();await release.promise;return new Response('private-result',{headers:{[marker]:'1','content-length':'14','content-encoding':'gzip'}});}});
...
it('withholds a completed query when its Better Auth session was revoked in flight',async()=>{
 ...
 const options:AuthOptions={env:{},cookieSecret:'test',tokens:{byToken:async()=>null,byId:async()=>null,invalidate:()=>{}},sessions:{resolve:async()=>loggedIn?{userId:'user',email:'user@example.com',emailVerified:true}:null},upstream:async()=>{started.resolve();await release.promise;return new Response('private-result',{headers:{[marker]:'1'}});}};
 const app=createAuthHost(options),response=app.request('http://localhost/a/abcdef/query');await started.promise;loggedIn=false;release.resolve();
...
it('keeps an authorized result and strips the internal marker',async()=>{
 ...
 const app=createAuthHost({env:{},cookieSecret:'test',tokens:{byToken:async()=>null,byId:async()=>null,invalidate:()=>{}},sessions:{resolve},upstream:async()=>new Response('result',{headers:{[marker]:'1'}})});
...
it('fails closed when the session recheck errors and does not recheck unmarked responses',async()=>{
 ...
 const base:AuthOptions={env:{},cookieSecret:'test',tokens:{byToken:async()=>null,byId:async()=>null,invalidate:()=>{}},sessions:{resolve},upstream:async()=>new Response('private',{headers:{[marker]:'1'}})};
 const refused=await createAuthHost(base).request('http://localhost/a/abcdef/query');expect(refused.status).toBe(401);expect(await refused.text()).not.toContain('private');
 calls=0;const normal=await createAuthHost({...base,upstream:async()=>new Response('normal')}).request('http://localhost/');expect(normal.status).toBe(200);expect(calls).toBe(1);
});
