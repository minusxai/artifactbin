...
it('resolves fresh identity for ordinary bearer requests and never accepts a mismatched account',async()=>{
 ...
 const proxy=createAuthHost(await testAuthOptions({sessions:{resolve:async()=>null,identity:async()=>{calls++;return identity;}},upstream:async(_request,actor)=>{seen=actor;return Response.json({ok:true});}}));
...
it('preserves ordinary token access when the identity lookup is unavailable',async()=>{
 ...
 const proxy=createAuthHost(await testAuthOptions({sessions:{resolve:async()=>null,identity:async()=>{throw new Error('unavailable');}},upstream:async(_request,actor)=>{seen=actor;return Response.json({ok:true});}}));
