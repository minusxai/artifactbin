...
describe('OIDC', () => {
  ...
  it('says the identity moments: a first IdP sign-in signs up, links and verifies; a later link says only the link and the login; an unverified email says nothing', async () => {
    ...
    expect(signedUp).toMatchObject({ source: 'auth', subject_kind: 'user', object_kind: 'user', payload: { email: 'fresh@example.com' } });
    expect(signedUp!.object_id).toMatch(/^usr_/);
    expect(linked).toMatchObject({ source: 'auth', verb: 'oauth_linked', object_id: signedUp!.object_id, payload: { provider: 'acme' } });
