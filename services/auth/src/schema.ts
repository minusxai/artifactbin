/** Identity owns two auth primitives: durable clients and lifecycle-managed credentials. */
import type { Queryable, Table } from '@artifactbin/contracts';
import { ensureTable } from '@artifactbin/utils';

export const AUTH_TABLES: Table[] = [
  {
    name: 'clients',
    columns: [
...
export async function ensureAuthSchema(db: Queryable, schema = 'auth'): Promise<void> {
  if (!/^[a-z_][a-z0-9_]*$/.test(schema)) throw new Error(`ensureAuthSchema: schema ${JSON.stringify(schema)} is not a plain identifier`);
  ...
  await ensureTable(db, AUTH_TABLES, { schema });
}
