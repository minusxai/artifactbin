...
export interface HumanAuthOptions {
  ...
  secret: string;
  baseURL: string;
  /** @deprecated Ignored for old embedding callers; baseURL is authoritative. */
...
export async function createHumanAuth(opts: HumanAuthOptions): Promise<HumanAuth> {
  ...
  return {
    handler: async request => {
      if (new URL(request.url).host !== main.host || cookieValues(request.headers, fullName).length > 1) {
    ...
    sessions: {
      async resolve(request) {
        if (new URL(request.url).host !== main.host || cookieValues(request.headers, fullName).length !== 1) return null;
