...
const html = (m: OutgoingMail): string =>
  m.kind === 'otp'
    ? `<div style="font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:15px;line-height:1.6;color:#0b0e11"><p>Your artifact-bin login code is</p><p style="font-size:32px;font-weight:600;letter-spacing:0.18em;margin:16px 0">${m.otp}</p><p style="color:#57606a">It expires in 10 minutes. If you didn't ask to log in, ignore this email.</p></div>`
...
export function resendMailer(opts: ResendOptions): Mailer {
  ...
  return {
    async send(mail) {
      ...
      const subject = mail.kind === 'otp' ? `${mail.otp} is your artifact-bin login code` : mail.subject;
      const text = mail.kind === 'otp' ? `Your artifact-bin login code is ${mail.otp}\n\nIt expires in 10 minutes. If you didn't ask to log in, ignore this email.` : mail.text;
