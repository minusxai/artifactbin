...
export type PairingResult = { status: 'pending' | 'invalid' | 'denied' } | { status: 'approved'; userId: string };
  ...
  return {
    ...
    async inspect(userCode: string, origin: string) {
      const result = await db.query(`SELECT 1 FROM ${table} WHERE kind = $1 AND group_id = $2
        ...
        AND deleted_at IS NULL AND payload->>'denied' IS DISTINCT FROM 'true' AND expires_at > now()`, [KIND, userCode, origin]);
    ...
    async approve(userCode: string, origin: string, userId: string): Promise<boolean> {
      ...
      const result = await db.query(`UPDATE ${table} SET subject_id = $4 WHERE kind = $1 AND group_id = $2
        AND payload->>'origin' = $3 AND subject_id IS NULL AND consumed_at IS NULL
        AND deleted_at IS NULL AND payload->>'denied' IS DISTINCT FROM 'true' AND expires_at > now() RETURNING credential_hash`, [KIND, userCode, origin, userId]);
    ...
    async deny(userCode: string, origin: string): Promise<boolean> {
      const result = await db.query(`UPDATE ${table} SET payload = payload || '{"denied":true}'::jsonb
        ...
        AND consumed_at IS NULL AND deleted_at IS NULL AND expires_at > now() RETURNING credential_hash`, [KIND,userCode,origin]);
      return result.rows.length === 1;
    },
    async consume(deviceCode: string, origin: string): Promise<PairingResult> {
      ...
      const approved = await db.query<{ subject_id: string }>(`UPDATE ${table} SET consumed_at = now()
        WHERE kind = $1 AND credential_hash = $2 AND payload->>'origin' = $3 AND subject_id IS NOT NULL
      ...
      if (approved.rows[0]) return { status: 'approved', userId: approved.rows[0].subject_id };
