...
const trusted={origin:controls,'x-artifactbin-csrf':'1'};
...
const adopt=async()=>{
  const response=await proxy.request(controls+'/api/session/token',{method:'POST',headers:trusted});
  expect(response.status).toBe(204);
  expect(response.headers.getSetCookie().some(c=>c.startsWith('__Secure-mx-agent-read='))).toBe(true);
  return response.headers.getSetCookie().map(c=>c.split(';')[0]).join('; ');
};
describe('anonymous browser authority on two hosts',()=>{
  it('issues read-only authority and revokes copied full/read cookies on disconnect without affecting another browser',async()=>{
    ...
    const account=async(cookie:string)=> (await proxy.request(controls+'/api/my/artifacts',{headers:{...trusted,cookie}})).json();
    expect((await read(one)).actor).toMatchObject({credential:'read-session',tokenId:'tok_browser'});
    ...
    expect((await proxy.request(controls+'/api/session/token',{method:'DELETE',headers:{...trusted,cookie:one}})).status).toBe(204);
    ...
    expect((await read(two)).actor).toMatchObject({credential:'read-session',tokenId:'tok_browser'});
