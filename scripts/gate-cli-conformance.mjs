...
try {
  ...
  record('CLI device login through host authentication');
  ...
  if (credentialSource) {
    ...
    const email = process.env.EVAL_LOGIN_EMAIL ?? `mxmx_test_conformance_${Date.now().toString(36)}@example.com`;
  ...
  sink.close();
  try { if (publishedIds.length) await invoke(['delete', ...new Set(publishedIds), '--force']); }
