...
try {
  ...
  document.querySelector('#increment').style.cssText='display:block;width:120px;height:40px';document.querySelector('#increment').onclick=async()=>{const snapshot=await mx.read(['count']);await mx.set({count:Number(snapshot.signals.count.value??0)+1})};mx.subscribe(['count'],snapshot=>document.querySelector('#increment').textContent='Count '+snapshot.signals.count.value);
