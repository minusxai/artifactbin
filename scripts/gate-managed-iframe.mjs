...
try {
  ...
  document.querySelector('button').style.cssText='display:block;width:120px;height:40px';document.querySelector('button').onclick=()=>mx.params.set('count',Number(mx.params.get('count')??0)+1);mx.params.subscribe(['count'],values=>document.querySelector('button').textContent='Count '+values.count);
  ...
  const markup='<Helmet><Value name="count" type="number" default={0}/></Helmet><p>Managed parent</p><Iframe title="Managed demo" height={200}><button>Increment</button><p id="result">waiting</p><canvas width="30" height="30"/><img src="'+cdn+'/image.png"/><script src="'+cdn+'/one.js"/><script type="module" src="'+cdn+'/two.js"/><script>{`'+author+'`}</script></Iframe><Iframe title="Second" height={100}><p>Second isolated region</p></Iframe>';
  ...
  for(let attempt=0;attempt<2;attempt++) {
    ...
    const outer=page.locator('iframe[title="Managed demo"]');await outer.waitFor();
    ...
    await realm.locator('button').click();await realm.locator('button').filter({hasText:'Count 1'}).waitFor();
  }
