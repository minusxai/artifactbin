...
import {instrumentClosedRoots} from './gate-browser.mjs';
...
export async function measureInteraction({page,base,controls,publish,engineName}) {
  ...
  await context.addInitScript(instrumentClosedRoots);
  ...
  const frame=measured.mainFrame();
  await frame.getByRole('button',{name:'Open artifact controls',exact:true}).waitFor();
  const readyMs=performance.now()-start;
  const output={engine:engineName,readyMs:+readyMs.toFixed(2),note:'localhost, warm serial writes, no CPU/network throttling; direct baseline runs in first-party main document, not an author sandbox bypass',operations:{}};
  ...
  assert(wire.every(url=>new URL(url).origin===base),'writes only reach trusted origin');
  output.mutationRequests=wire.length;
  output.allWritesOnMainOrigin=true;
