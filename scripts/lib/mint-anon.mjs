 ...
 * to anything that is not a browser (the `browser_only: true` route in the proxy's policy file): an agent that mints its own token publishes
 ...
 * fourteen places to forget when the rule changes, and the first symptom would be a 403 in an unrelated gate.
 *
 * `/api/start` does NOT carry that flag and needs no headers — it shares only the `anon_mint` budget.
 */
...
/** The raw Response, for a caller that wants to read the status itself (a 429 is the budget, a 403 the browser rule). */
...
export async function mintAnon(base, init = {}) {
  ...
  if (!res.ok || !body?.token) {
    const hint = res.status === 429
      ...
      : res.status === 403
        ? ' (the browser-context rule — this helper should have carried the page\'s headers)'
        : '';
