 ...
 *                 schema module), a FRESH-database / CI artifact
 *   roles.sql   — the two LOGIN roles and the two schemas they OWN (Addendum 2:
 *                 AUTHORIZATION, not grants — USAGE-only left the app role
 *                 unable to run its own DDL)
 *   grants.sql  — the ONE cross-schema grant: the proxy's SELECT on app.tokens,
 *                 applied AFTER the app's first boot creates the table
 *
...
function render() {
  return Promise.all([appSide(), proxySide(), betterAuthSide()]).then(([app, proxy, betterAuth]) => {
    const schema = [
