...
check(anonGet.status === 404, `the document's own GET is the uniform 404 for a private document (${anonGet.status})`);
...
check(nope.status === 404, 'an unknown id is the uniform 404 (POST)');
check((await fetch(`${B}/a/zzzzzz/query?q=%7B%7D`)).status === 404, 'an unknown id is the uniform 404 (GET)');
