...
import {AUTHOR_FRAME_DOCUMENT} from '../../services/app/lib/story-runtime/author-frame.ts';
...
const server=createServer((req,res)=>{res.setHeader('Content-Type','text/html');if(req.url==='/wrapper'){res.setHeader('Content-Security-Policy',policy+';sandbox allow-scripts');res.end(AUTHOR_FRAME_DOCUMENT);return;}res.end(req.url==='/control'?'<body><script>addEventListener("message",e=>window.result=e.data.rtc);'+script+'</script>':'<body><script>addEventListener("message",e=>window.result=e.data.rtc);const f=document.createElement("iframe");f.sandbox="allow-scripts";f.src="/wrapper";f.onload=()=>{const channel=new MessageChannel();f.contentWindow.postMessage({type:"mx:author:init",document:'+JSON.stringify(inner).replaceAll('<','\\u003c')+'},"*",[channel.port2]);};document.body.append(f)</script>');});
