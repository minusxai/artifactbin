...
try {
 const child=spawn(process.execPath,['--import','tsx',resolve('scripts/experiments/single-row-worker.mjs'),...process.argv.slice(2)],{stdio:'inherit',env:{...process.env,NODE_ENV:'development',DATABASE_URL:`postgresql://probe@localhost:5490/postgres?host=${encodeURIComponent(cluster)}`,APP__PUBLIC_BASE_URL:'http://localhost:5430',EVENTS__SERVICE_URL:'',SQL__SERVICE_URL:'',BROWSER__SERVICE_URL:''}});
 const code=await new Promise((resolve,reject)=>{child.on('exit',resolve);child.on('error',reject);});if(code)process.exitCode=code;
