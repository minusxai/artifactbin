...
try {
 const worker=process.argv.includes('--semantic-app')?'semantic-artifact-worker.mjs':process.argv.includes('--semantic-benchmark')?'semantic-kernel-benchmark.mjs':process.argv.includes('--semantic-kernel')?'semantic-kernel-worker.mjs':process.argv.includes('--validity')?'validated-operations-worker.mjs':'single-row-worker.mjs';
