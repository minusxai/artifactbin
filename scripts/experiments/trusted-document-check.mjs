...
import {prepareClientDocumentUpdate} from '../../services/app/lib/story/document-update-client.ts';
...
import {applyGraphPatch} from '../../services/app/lib/story/document-graph-patch.ts';
...
try{
 ...
 const verify=async()=>{
  ...
  for(const {document_state:s} of logs){graph=applyGraphPatch(graph,version++,s.forward);assert.ok(graph);assert.deepEqual(graphIntegrity(graph),[]);}
