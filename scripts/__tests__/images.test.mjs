...
describe('docker-install-layer', () => {
  ...
  describe('every install layer', () => {
    ...
    it('finds the installs to judge (the scan is not vacuous)', () => {
      expect(files).toEqual(['Dockerfile']);
...
describe('npm-ci-no-audit', () => {
  ...
  describe('every npm ci in every image', () => {
    ...
    it('finds the single host image (the scan is not vacuous)', () => {
      expect(files).toContain('Dockerfile');
      expect(files).toEqual(['Dockerfile']);
      expect(files.flatMap(npmCiLines).length).toBe(2);
    });
...
describe('OSS single-host composition', () => {
  it('owns one host with durable PGLite and object storage, without a proxy or external database', () => {
    const composition = yaml.parse(readFileSync(path.join(REPO_ROOT, 'docker-compose.yml'), 'utf8'));
    expect(Object.keys(composition.services)).toEqual(['web']);
    expect(composition.services.web.environment.DATABASE_URL).toBe('pglite:///app/data/pglite');
    expect(composition.services.web.environment.OBJECT_STORE__LOCAL_DIR).toBe('/app/data/objects');
    expect(composition.services.web.volumes).toContain('artifactbin_data:/app/data');
    expect(composition.services.web.environment).not.toHaveProperty('PROXY__RATE_LIMIT_CONFIG_FILE');
    expect(composition.services.web.environment).not.toHaveProperty('S3_URL');
  });
});
 ...
 describe('public authentication assembly', () => {
  it('registers auth without a proxy workspace or rate-limit exports', () => {
    ...
    expect(readFileSync(path.join(REPO_ROOT, 'Dockerfile'), 'utf8')).not.toContain('services/proxy');
  });
});
