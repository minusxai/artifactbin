...
import {runInvocation, checkoutIsolationRoots, agentEnvironment, taskTempEnvironment } from './lib/spawn';
...
async function runTask(r: TaskRun): Promise<Outcome> {
  ...
  const spawned = await runInvocation({ ...adapter.invocation(ctx), redact: [r.apiKey] }, {
    ...
    baseEnv: { ...agentEnvironment(process.env), ...taskTempEnvironment(homeDir), ...r.agentEnv, PATH:[browserShims(path.join(homeDir, 'shims')), cliBin ?? path.join(homeDir, '.local', 'bin'),r.agentEnv.PATH??process.env.PATH??''].join(path.delimiter) },
